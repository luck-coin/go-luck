...
// Package bind generates Luck contract Go bindings.
//
// Detailed usage document and tutorial available on the go-luck Wiki page:
// https://github.com/luck/go-luck/wiki/Native-DApps:-Go-bindings-to-Luck-contracts
package bind
...
func bindStructTypeGo(kind abi.Type, structs map[string]*tmplStruct) string {
	...
	case abi.TupleTy:
		...
		// tolucker here. The reason is before solidity v0.5.11, kind.TupleRawName
		// is empty, so we use canonical parameter expression to distinguish
		// different struct definition. From the consideration of backward
		// compatibility, we concat these two tolucker so that if kind.TupleRawName
...
// structured checks whforter a list of ABI data types has enough information to
...
// hasStruct returns an indicator whforter the given type is struct, struct slice
// or struct array.
