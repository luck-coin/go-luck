...
type Wallet struct {
	...
	deriveChain     luck.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *Wallet) Open(passphrase string) error {
	...
	if !w.session.paired() {
		...
		if pairing := w.Hub.pairing(w); pairing != nil {
			...
			// but we can't return ErrPINNeeded directly here becase we don't know whforter
			// a PIN check or a PIN reset is needed.
			passphrase = ""
		}
	}
	// The smart card was successfully paired, retrieve its status to check whforter
	...
	for errc == nil && err == nil {
		...
		for i := 0; i < len(nextAddrs); i++ {
			for empty := false; !empty; {
				// Retrieve the next derived Luck account
...
// Contains returns whforter an account is part of this particular wallet or not.
...
// chain state reader.
func (w *Wallet) SelfDerive(bases []accounts.DerivationPath, chain luck.ChainStateReader) {
	w.lock.Lock()
	defer w.lock.Unlock()
...
type Session struct {
	...
	verified bool                  // Whforter the pin has been verified in this session.
}
...
type walletStatus struct {
	...
	Initialized   bool // Whforter the card has been initialized with a private key
}
