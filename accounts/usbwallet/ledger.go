...
// https://raw.githubusercontent.com/LedgerHQ/blue-app-fort/master/doc/fortapp.asc
...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and Luck address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an Luck transaction after having the user validate the parameters
...
type ledgerDriver struct {
	...
	browser bool          // Flag whforter the Ledger is in browser mode (reply channel mismatch)
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "Luck app in browser mode", w.failure
	}
	if w.offline() {
		return "Luck app offline", w.failure
	}
	return fmt.Sprintf("Luck app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}

// offline returns whforter the wallet and the Luck app is offline or not.
//
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// Luck app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the Luck app's version, will fail prior to v1.0.2
...
// and returning the Luck address located on that derivation path.
...
//
// Note, if the version of the Luck application running on the Ledger wallet is
// too old to sign EIP-155 transactions, but such is requested nonforteless, an error
// will be returned opposed to silently signing in Homestead mode.
func (w *ledgerDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	// If the Luck app doesn't run, abort
	if w.offline() {
...
// ledgerVersion retrieves the current version of the Luck wallet app running
...
// ledgerDerive retrieves the currently active Luck address from a Ledger
...
//   Luck address length | 1 byte
//   Luck address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the Luck hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an Luck address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Create the transaction RLP based on whforter legacy or EIP155 signing was requested
	var (
		txrlp []byte
		err   error
	)
	...
	// Extract the Luck signature and do a sanity validation
