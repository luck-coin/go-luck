...
import (
	...
	"github.com/luck/go-luck/accounts/usbwallet/trezor"
...
type trezorDriver struct {
	...
	pinwait        bool          // Flags whforter the device is waiting for PIN entry
	passphrasewait bool          // Flags whforter the device is waiting for passphrase entry
...
// Status implements accounts.Wallet, always whforter the Trezor is opened, closed
// or whforter the Luck app was not started on it.
...
// Luck address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.LuckAddress)
	if _, err := w.trezorExchange(&trezor.LuckGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.LuckTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.LuckTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
