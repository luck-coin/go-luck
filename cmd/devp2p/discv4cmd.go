...
func discv4ResolveJSON(ctx *cli.Context) error {
	...
	disc := startV4(ctx)
	defer disc.Close()
	c := newCrawler(inputSet, disc, lnode.IterNodes(nodeargs))
...
func makeDiscoveryConfig(ctx *cli.Context) (*lnode.LocalNode, discover.Config) {
...
func listen(ln *lnode.LocalNode, addr string) *net.UDPConn {
	if addr == "" {
		addr = "0.0.0.0:0"
	}
...
func parseBootnodes(ctx *cli.Context) ([]*lnode.Node, error) {
	s := params.TestnetBootnodes
	if ctx.IsSet(bootnodesFlag.Name) {
		s = strings.Split(ctx.String(bootnodesFlag.Name), ",")
	}
	nodes := make([]*lnode.Node, len(s))
	var err error
