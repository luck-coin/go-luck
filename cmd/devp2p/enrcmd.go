...
func dumpRecord(r *enr.Record) string {
	...
	if n, err := lnode.New(lnode.ValidSchemes, r); err != nil {
...
func parseNode(source string) (*lnode.Node, error) {
	if strings.HasPrefix(source, "lnode://") {
		return lnode.ParseV4(source)
	}
	r, err := parseRecord(source)
	if err != nil {
		return nil, err
	}
	return lnode.New(lnode.ValidSchemes, r)
}
