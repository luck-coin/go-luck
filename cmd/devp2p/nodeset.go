...
// as a JSON object.
type nodeSet map[lnode.ID]nodeJSON

type nodeJSON struct {
	Seq uint64      `json:"seq"`
	N   *lnode.Node `json:"record"`
...
func (ns nodeSet) nodes() []*lnode.Node {
	result := make([]*lnode.Node, 0, len(ns))
	for _, n := range ns {
...
func (ns nodeSet) add(nodes ...*lnode.Node) {
	for _, n := range nodes {
