...
import (
	...
	"github.com/luck/go-luck/cmd/utils"
	...
	"github.com/luck/go-luck/consensus/misc"
...
var (
	...
	retestfortCommand = cli.Command{
		Action:      utils.MigrateFlags(retestfort),
		Name:        "retestfort",
		Usage:       "Launches luck in retestfort mode",
		ArgsUsage:   "",
		Flags:       []cli.Flag{rpcPortFlag},
		Category:    "MISCELLANEOUS COMMANDS",
		Description: `Launches luck in retestfort mode (no database, no network, only retestfort RPC interface)`,
	}
)

type RetestfortTestAPI interface {
...
type RetestfortEthAPI interface {
...
type RetestfortDebugAPI interface {
...
type RetestfortAPI struct {
	fortDb         fortdb.Database
	db            state.Database
...
func (api *RetestfortAPI) SetChainParams(ctx context.Context, chainParams ChainParams) (bool, error) {
	// Clean up
	...
	fortDb := rawdb.NewMemoryDatabase()
	...
	chainConfig, genesisHash, err := core.SetupGenesisBlock(fortDb, genesis)
	if err != nil {
		return false, err
	}
	...
	blockchain, err := core.NewBlockChain(fortDb, nil, chainConfig, engine, vm.Config{}, nil)
	if err != nil {
		return false, err
	}
	...
	api.db = state.NewDatabase(api.fortDb)
...
func (api *RetestfortAPI) SendRawTransaction(ctx context.Context, rawTx hexutil.Bytes) (common.Hash, error) {
...
func (api *RetestfortAPI) MineBlocks(ctx context.Context, number uint64) (bool, error) {
...
func (api *RetestfortAPI) currentNumber() uint64 {
...
func (api *RetestfortAPI) mineBlock() error {
	number := api.currentNumber()
	parentHash := rawdb.ReadCanonicalHash(api.fortDb, number)
	parent := rawdb.ReadBlock(api.fortDb, parentHash, number)
	...
	// If we are care about TheDAO hard-fork check whforter to override the extra-data or not
	if daoBlock := api.chainConfig.DAOForkBlock; daoBlock != nil {
		// Check whforter the block is among the fork extra-override range
		limit := new(big.Int).Add(daoBlock, params.DAOForkExtraRange)
		if header.Number.Cmp(daoBlock) >= 0 && header.Number.Cmp(limit) < 0 {
			// Depending whforter we support or oppose the fork, override differently
...
func (api *RetestfortAPI) importBlock(block *types.Block) error {
...
func (api *RetestfortAPI) ModifyTimestamp(ctx context.Context, interval uint64) (bool, error) {
	api.blockInterval = interval
	return true, nil
}

func (api *RetestfortAPI) ImportRawBlock(ctx context.Context, rawBlock hexutil.Bytes) (common.Hash, error) {
...
func (api *RetestfortAPI) RewindToBlock(ctx context.Context, newHead uint64) (bool, error) {
...
func (api *RetestfortAPI) GetLogHash(ctx context.Context, txHash common.Hash) (common.Hash, error) {
	receipt, _, _, _ := rawdb.ReadReceipt(api.fortDb, txHash, api.chainConfig)
	if receipt == nil {
...
func (api *RetestfortAPI) BlockNumber(ctx context.Context) (uint64, error) {
	return api.currentNumber(), nil
}

func (api *RetestfortAPI) GetBlockByNumber(ctx context.Context, blockNr math.HexOrDecimal64, fullTx bool) (map[string]interface{}, error) {
...
func (api *RetestfortAPI) GetBlockByHash(ctx context.Context, blockHash common.Hash, fullTx bool) (map[string]interface{}, error) {
...
func (api *RetestfortAPI) AccountRange(ctx context.Context,
	blockHashOrNumber *math.HexOrDecimal256, txIndex uint64,
	addressHash *math.HexOrDecimal256, maxResults uint64,
) (AccountRangeResult, error) {
	var (
...
func (api *RetestfortAPI) GetBalance(ctx context.Context, address common.Address, blockNr math.HexOrDecimal64) (*math.HexOrDecimal256, error) {
...
func (api *RetestfortAPI) GetCode(ctx context.Context, address common.Address, blockNr math.HexOrDecimal64) (hexutil.Bytes, error) {
	header := api.blockchain.GetHeaderByNumber(uint64(blockNr))
	statedb, err := api.blockchain.StateAt(header.Root)
	if err != nil {
		return nil, err
...
func (api *RetestfortAPI) GetTransactionCount(ctx context.Context, address common.Address, blockNr math.HexOrDecimal64) (uint64, error) {
	header := api.blockchain.GetHeaderByNumber(uint64(blockNr))
	statedb, err := api.blockchain.StateAt(header.Root)
	if err != nil {
		return 0, err
...
func (api *RetestfortAPI) StorageRangeAt(ctx context.Context,
	blockHashOrNumber *math.HexOrDecimal256, txIndex uint64,
	address common.Address,
	begin *math.HexOrDecimal256, maxResults uint64,
) (StorageRangeResult, error) {
...
func (api *RetestfortAPI) ClientVersion(ctx context.Context) (string, error) {
	return "Gfort-" + params.VersionWithCommit(gitCommit, gitDate), nil
}
...
func retestfort(ctx *cli.Context) error {
	log.Info("Welcome to retestfort!")
	// register signer API with server
	var (
		extapiURL string
	)
	apiImpl := &RetestfortAPI{}
	var testApi RetestfortTestAPI = apiImpl
	var fortApi RetestfortEthAPI = apiImpl
	var debugApi RetestfortDebugAPI = apiImpl
	...
	srv := rpc.NewServer()
	err := node.RegisterApisFromWhitelist(rpcAPI, []string{"test", "fort", "debug", "web3"}, srv, false)
	if err != nil {
	...
	// start http server
	var RetestfortHTTPTimeouts = rpc.HTTPTimeouts{
		ReadTimeout:  120 * time.Second,
		WriteTimeout: 120 * time.Second,
		IdleTimeout:  120 * time.Second,
	}
	httpEndpoint := fmt.Sprintf("%s:%d", ctx.GlobalString(utils.RPCListenAddrFlag.Name), ctx.Int(rpcPortFlag.Name))
	listener, err := node.StartHTTPEndpoint(httpEndpoint, RetestfortHTTPTimeouts, handler)
	if err != nil {
