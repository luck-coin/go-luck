...
import (
	...
	math2 "github.com/luck/go-luck/common/math"
...
// alfortGenesisSpec represents the genesis specification format used by the
// C++ Luck implementation.
type alfortGenesisSpec struct {
	...
	Accounts map[common.UnprefixedAddress]*alfortGenesisSpecAccount `json:"accounts"`
}

// alfortGenesisSpecAccount is the prefunded genesis account and/or precompiled
// contract definition.
type alfortGenesisSpecAccount struct {
	Balance     *math2.HexOrDecimal256   `json:"balance,omitempty"`
	Nonce       uint64                   `json:"nonce,omitempty"`
	Precompiled *alfortGenesisSpecBuiltin `json:"precompiled,omitempty"`
}

// alfortGenesisSpecBuiltin is the precompiled contract definition.
type alfortGenesisSpecBuiltin struct {
	Name          string                         `json:"name,omitempty"`
	StartingBlock *hexutil.Big                   `json:"startingBlock,omitempty"`
	Linear        *alfortGenesisSpecLinearPricing `json:"linear,omitempty"`
}

type alfortGenesisSpecLinearPricing struct {
	Base uint64 `json:"base"`
	Word uint64 `json:"word"`
}

// newAlfortGenesisSpec converts a go-luck genesis block into a Alfort-specific
// chain specification format.
func newAlfortGenesisSpec(network string, genesis *core.Genesis) (*alfortGenesisSpec, error) {
	// Only ethash is currently supported between go-luck and alfort
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	// Reconstruct the chain spec in Alfort format
	spec := &alfortGenesisSpec{
		SealEngine: "Ethash",
	}
	// Some defaults
	...
	// config but alfort will sync with ETC clients up until the actual dao hard
	// fork block.
	...
	spec.setPrecompile(1, &alfortGenesisSpecBuiltin{Name: "ecrecover",
		Linear: &alfortGenesisSpecLinearPricing{Base: 3000}})
	spec.setPrecompile(2, &alfortGenesisSpecBuiltin{Name: "sha256",
		Linear: &alfortGenesisSpecLinearPricing{Base: 60, Word: 12}})
	spec.setPrecompile(3, &alfortGenesisSpecBuiltin{Name: "ripemd160",
		Linear: &alfortGenesisSpecLinearPricing{Base: 600, Word: 120}})
	spec.setPrecompile(4, &alfortGenesisSpecBuiltin{Name: "identity",
		Linear: &alfortGenesisSpecLinearPricing{Base: 15, Word: 3}})
	if genesis.Config.ByzantiumBlock != nil {
		spec.setPrecompile(5, &alfortGenesisSpecBuiltin{Name: "modexp",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock)})
		spec.setPrecompile(6, &alfortGenesisSpecBuiltin{Name: "alt_bn128_G1_add",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
			Linear:        &alfortGenesisSpecLinearPricing{Base: 500}})
		spec.setPrecompile(7, &alfortGenesisSpecBuiltin{Name: "alt_bn128_G1_mul",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
			Linear:        &alfortGenesisSpecLinearPricing{Base: 40000}})
		spec.setPrecompile(8, &alfortGenesisSpecBuiltin{Name: "alt_bn128_pairing_product",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock)})
	}
	if genesis.Config.IstanbulBlock != nil {
		...
		spec.setPrecompile(6, &alfortGenesisSpecBuiltin{
			Name:          "alt_bn128_G1_add",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
		}) // Alfort hardcoded the gas policy
		spec.setPrecompile(7, &alfortGenesisSpecBuiltin{
			Name:          "alt_bn128_G1_mul",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
		}) // Alfort hardcoded the gas policy
		spec.setPrecompile(9, &alfortGenesisSpecBuiltin{
...
func (spec *alfortGenesisSpec) setPrecompile(address byte, data *alfortGenesisSpecBuiltin) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*alfortGenesisSpecAccount)
	}
	addr := common.UnprefixedAddress(common.BytesToAddress([]byte{address}))
	if _, exist := spec.Accounts[addr]; !exist {
		spec.Accounts[addr] = &alfortGenesisSpecAccount{}
	}
	spec.Accounts[addr].Precompiled = data
}

func (spec *alfortGenesisSpec) setAccount(address common.Address, account core.GenesisAccount) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*alfortGenesisSpecAccount)
	}
...
type parityChainSpecPricing struct {
	...
	// Before the https://github.com/paritytech/parity-luck/pull/11039,
...
// newParityChainSpec converts a go-luck genesis block into a Parity specific
// chain specification format.
func newParityChainSpec(network string, genesis *core.Genesis, bootnodes []string) (*parityChainSpec, error) {
	// Only ethash is currently supported between go-luck and Parity
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	...
	// https://github.com/luck/EIPs/blob/master/EIPS/eip-608.md
	spec.Params.EIP150Transition = hexutil.Uint64(genesis.Config.EIP150Block.Uint64())

	// Spurious Dragon: 155, 160, 161, 170
	// https://github.com/luck/EIPs/blob/master/EIPS/eip-607.md
	...
	// luck has it set from zero
	spec.Params.MaxCodeSizeTransition = 0

	// Disable this one
	spec.Params.EIP98Transition = math.MaxInt64

	spec.Genesis.Seal.Luck.Nonce = types.EncodeNonce(genesis.Nonce)
	spec.Genesis.Seal.Luck.MixHash = (genesis.Mixhash[:])
	spec.Genesis.Difficulty = (*hexutil.Big)(genesis.Difficulty)
	spec.Genesis.Author = genesis.Coinbase
	spec.Genesis.Timestamp = (hexutil.Uint64)(genesis.Timestamp)
	spec.Genesis.ParentHash = genesis.ParentHash
...
// pyLuckGenesisSpec represents the genesis specification format used by the
// Python Luck implementation.
type pyLuckGenesisSpec struct {
	Nonce      types.BlockNonce  `json:"nonce"`
	Timestamp  hexutil.Uint64    `json:"timestamp"`
	ExtraData  hexutil.Bytes     `json:"extraData"`
	GasLimit   hexutil.Uint64    `json:"gasLimit"`
...
// newPyLuckGenesisSpec converts a go-luck genesis block into a Parity specific
// chain specification format.
func newPyLuckGenesisSpec(network string, genesis *core.Genesis) (*pyLuckGenesisSpec, error) {
	// Only ethash is currently supported between go-luck and pyluck
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	spec := &pyLuckGenesisSpec{
