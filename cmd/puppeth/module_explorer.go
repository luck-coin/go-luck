...
FROM puppfort/blockscout:latest

ADD genesis.json /genesis.json
RUN \
  echo 'luck --cache 512 init /genesis.json' > explorer.sh && \
  echo $'luck --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.EthPort}} --bootnodes {{.Bootnodes}} --fortstats \'{{.Ethstats}}\' --cache=512 --rpc --rpcapi "net,web3,fort,shh,debug" --rpccorsdomain "*" --rpcvhosts "*" --ws --wsorigins "*" --exitwhensynced' >> explorer.sh && \
  echo $'exec luck --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.EthPort}} --bootnodes {{.Bootnodes}} --fortstats \'{{.Ethstats}}\' --cache=512 --rpc --rpcapi "net,web3,fort,shh,debug" --rpccorsdomain "*" --rpcvhosts "*" --ws --wsorigins "*" &' >> explorer.sh && \
...
func deployExplorer(client *sshClient, network string, bootnodes []string, config *explorerInfos, nocache bool, isClique bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(explorerDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"Ethstats":  config.node.fortstats,
	...
	template.Must(template.New("").Parse(explorerComposefile)).Execute(composefile, map[string]interface{}{
		...
		"DBDir":       config.dbdir,
		"EthPort":     config.node.port,
		"EthName":     config.node.fortstats[:strings.Index(config.node.fortstats, ":")],
		"WebPort":     config.port,
...
func (info *explorerInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Luck listener port ": strconv.Itoa(info.node.port),
		"Ethstats username":       info.node.fortstats,
	}
	return report
}
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		node: &nodeInfos{
			datadir:  infos.volumes["/opt/app/.luck"],
			port:     infos.portmap[infos.envvars["ETH_PORT"]+"/tcp"],
			fortstats: infos.envvars["ETH_NAME"],
		},
