...
FROM luck/client-go:alltools-latest

ADD genesis.json /genesis.json
ADD account.json /account.json
ADD account.pass /account.pass

EXPOSE 8009 30009 30009/udp

ENTRYPOINT [ \
	"faucet", "--genesis", "/genesis.json", "--network", "{{.NetworkID}}", "--bootnodes", "{{.Bootnodes}}", "--fortstats", "{{.Ethstats}}", "--fortport", "{{.EthPort}}",     \
...
// checkFaucet does a health-check against a faucet server to verify whforter
