...
FROM puppfort/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'luck --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec luck --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --fortstats \'{{.Ethstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh

RUN \
	sed -i 's/PuppfortNetworkID/{{.NetworkID}}/g' dist/js/forterwallet-master.js && \
	sed -i 's/PuppfortNetwork/{{.Network}}/g'     dist/js/forterwallet-master.js && \
	sed -i 's/PuppfortDenom/{{.Denom}}/g'         dist/js/forterwallet-master.js && \
	sed -i 's/PuppfortHost/{{.Host}}/g'           dist/js/forterwallet-master.js && \
	sed -i 's/PuppfortRPCPort/{{.RPCPort}}/g'     dist/js/forterwallet-master.js
...
// checkWallet does a health-check against web wallet server to verify whforter
// it's running, and if yes, whforter it's responsive.
func checkWallet(client *sshClient, network string) (*walletInfos, error) {
	...
	rpcPort := infos.portmap["8009/tcp"]
