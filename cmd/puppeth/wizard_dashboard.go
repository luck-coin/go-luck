...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"fortstats", "explorer", "wallet", "faucet"} {
	...
	// If we have fortstats running, ask whforter to make the secret public or not
	if w.conf.fortstats != "" {
		fmt.Println()
		fmt.Println("Include fortstats secret on dashboard (y/n)? (default = yes)")
