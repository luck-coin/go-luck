...
func (w *wizard) deployExplorer() {
	...
	fmt.Println()
	if infos.node.fortstats == "" {
		fmt.Printf("What should the explorer be called on the stats page?\n")
		infos.node.fortstats = w.readString() + ":" + w.conf.fortstats
	} else {
		fmt.Printf("What should the explorer be called on the stats page? (default = %s)\n", infos.node.fortstats)
		infos.node.fortstats = w.readDefaultString(infos.node.fortstats) + ":" + w.conf.fortstats
	}
