...
// importGenesis imports a Gfort genesis spec into puppfort.
...
func (w *wizard) manageGenesis() {
	// Figure out whforter to modify or export the genesis
	fmt.Println()
	...
	case "2":
		...
		fmt.Printf("  Will create %s.json, %s-alfort.json, %s-harmony.json, %s-parity.json\n", w.network, w.network, w.network, w.network)
		...
		// Export the native genesis spec used by puppfort and Gfort
		luckJson := filepath.Join(folder, fmt.Sprintf("%s.json", w.network))
		if err := ioutil.WriteFile((luckJson), out, 0644); err != nil {
			log.Error("Failed to save genesis file", "err", err)
			return
		}
		log.Info("Saved native genesis chain spec", "path", luckJson)

		// Export the genesis spec used by Alfort (formerly C++ Luck)
		if spec, err := newAlfortGenesisSpec(w.network, w.conf.Genesis); err != nil {
			log.Error("Failed to create Alfort chain spec", "err", err)
		} else {
			saveGenesis(folder, w.network, "alfort", spec)
		}
		...
		// Export the genesis spec used by Harmony (formerly LuckJ)
	...
	case "3":
		...
		w.conf.flush()
	default:
		log.Error("That's not somforting I can do")
		return
	}
}
