...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for fortstats availability")
	if infos, err := checkEthstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["fortstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["fortstats"] = infos.Report()
		fortstats = infos.config
	}
	...
	if infos, err := checkNode(client, w.network, true); err != nil {
		...
		bootnodes = append(bootnodes, infos.lnode)
	}
