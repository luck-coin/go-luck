...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.fortstats == "" {
		log.Error("No fortstats server configured")
		return
	}
	...
	if err != nil {
		if boot {
			infos = &nodeInfos{port: 30009, peersTotal: 512, peersLight: 256}
		} else {
			infos = &nodeInfos{port: 30009, peersTotal: 50, peersLight: 0, gasTarget: 7.5, gasLimit: 10, gasPrice: 1}
		}
	}
	existed := err == nil
	...
	if infos.fortstats == "" {
		...
		infos.fortstats = w.readString() + ":" + w.conf.fortstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.fortstats)
		infos.fortstats = w.readDefaultString(infos.fortstats) + ":" + w.conf.fortstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.Ethash != nil {
			// Ethash based miners only need an forterbase to mine against
			fmt.Println()
			if infos.forterbase == "" {
				fmt.Printf("What address should the miner use?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.forterbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner use? (default = %s)\n", infos.forterbase)
				infos.forterbase = w.readDefaultAddress(common.HexToAddress(infos.forterbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy Luck node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
