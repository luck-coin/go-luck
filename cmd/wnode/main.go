...
import (
	...
	"github.com/luck/go-luck/whisper/mailserver"
...
var (
	...
	argIP      = flag.String("ip", "", "IP address and port of this node (e.g. 127.0.0.1:30009)")
	argPub     = flag.String("pub", "", "public key for asymmetric encryption")
	argDBPath  = flag.String("dbpath", "", "path to the server's DB directory")
	argIDFile  = flag.String("idfile", "", "file name with node id (private key)")
	argEnode   = flag.String("boot", "", "bootstrap node you want to connect to (e.g. lnode://e454......08d50@52.176.211.200:16428)")
...
func processArgs() {
	...
	const lnodePrefix = "lnode://"
	if len(*argEnode) > 0 {
		if (*argEnode)[:len(lnodePrefix)] != lnodePrefix {
			*argEnode = lnodePrefix + *argEnode
		}
	}
...
func initialize() {
	...
	if *bootstrapMode {
		...
		*bootstrapMode = true
		...
		if len(*argEnode) == 0 {
			argEnode = scanLineA("Please enter the peer's lnode: ")
		}
		peer := lnode.MustParse(*argEnode)
...
func extractIDFromEnode(s string) []byte {
	n, err := lnode.Parse(lnode.ValidSchemes, s)
	if err != nil {
