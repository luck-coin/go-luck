...
// mapped from textual names used in the tests below to actual Luck private
...
// address retrieves the Luck address of a tester account by label, creating
...
func (ap *testerAccountPool) address(account string) common.Address {
	...
	// Resolve and return the Luck address
...
func TestClique(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			// batch (https://github.com/luck/go-luck/issues/17593). Whilst this
			// seems overly specific and weird, it was a Testnet consensus split.
			epoch:   3,
			signers: []string{"A", "B", "C"},
			votes: []testerVote{
				{signer: "A"},
	...
	for i, tt := range tests {
		...
		for j, block := range blocks {
			// Gfort the header and prepare it for signing
			header := block.Header()
			if j > 0 {
