...
import (
	...
	"github.com/luck/go-luck/internal/jsre"
...
type bridge struct {
	client   *rpc.Client  // RPC client to execute Luck requests through
...
func getJfort(vm *goja.Runtime) *goja.Object {
	jfort := vm.Get("jfort")
	if jfort == nil {
		panic(vm.ToValue("jfort object does not exist"))
	}
	return jfort.ToObject(vm)
}

// NewAccount is a wrapper around the personal.newAccount RPC method that uses a
// non-echoing password prompt to acquire the passphrase and executes the original
// RPC method (saved in jfort.newAccount) with it to actually execute the RPC call.
func (b *bridge) NewAccount(call jsre.Call) (goja.Value, error) {
	...
	newAccount, callable := goja.AssertFunction(getJfort(call.VM).Get("newAccount"))
	if !callable {
		return nil, fmt.Errorf("jfort.newAccount is not callable")
	}
...
func (b *bridge) OpenWallet(call jsre.Call) (goja.Value, error) {
	...
	openWallet, callable := goja.AssertFunction(getJfort(call.VM).Get("openWallet"))
	if !callable {
		return nil, fmt.Errorf("jfort.openWallet is not callable")
	}
...
// original RPC method (saved in jfort.unlockAccount) with it to actually execute
// the RPC call.
func (b *bridge) UnlockAccount(call jsre.Call) (goja.Value, error) {
	...
	unlockAccount, callable := goja.AssertFunction(getJfort(call.VM).Get("unlockAccount"))
	if !callable {
		return nil, fmt.Errorf("jfort.unlockAccount is not callable")
	}
...
// jfort.sign) with it to actually execute the RPC call.
func (b *bridge) Sign(call jsre.Call) (goja.Value, error) {
	...
	// Send the request to the backend and return
	sign, callable := goja.AssertFunction(getJfort(call.VM).Get("unlockAccount"))
	if !callable {
		return nil, fmt.Errorf("jfort.unlockAccount is not callable")
	}
...
func (b *bridge) SleepBlocks(call jsre.Call) (goja.Value, error) {
	...
	for time.Now().Before(deadline) {
		...
		err := b.client.Call(&number, "fort_blockNumber")
		if err != nil {
			return nil, err
		}
