...
import (
	...
	"github.com/luck/go-luck/internal/jsre/deps"
	"github.com/luck/go-luck/internal/web3ext"
...
func (c *Console) initExtensions() error {
	...
	aliases := map[string]struct{}{"fort": {}, "personal": {}}
	for api := range apis {
		if api == "web3" {
			continue
		}
...
// interaction. The original web3 callbacks are stored in 'jfort'. These will be called
...
func (c *Console) initPersonal(vm *goja.Runtime, bridge *bridge) {
	...
	jfort := vm.NewObject()
	vm.Set("jfort", jfort)
	jfort.Set("openWallet", personal.Get("openWallet"))
	jfort.Set("unlockAccount", personal.Get("unlockAccount"))
	jfort.Set("newAccount", personal.Get("newAccount"))
	jfort.Set("sign", personal.Get("sign"))
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines fort.getBalance(fort.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Gfort instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	message := "Welcome to the Luck JavaScript console!\n\n"

	// Print some generic Gfort metadata
	if res, err := c.jsre.Run(`
		var message = "instance: " + web3.version.node + "\n";
		try {
			message += "coinbase: " + luck.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + luck.blockNumber + " (" + new Date(1000 * fort.getBlock(fort.blockNumber).timestamp) + ")\n";
		try {
...
func (c *Console) Interactive() {
	...
	// the signal, a signal can still be received for unsupported terminals. Unlucktely
