...
func newTester(t *testing.T, confOverride func(*fort.Config)) *tester {
	...
	// Create a networkless protocol stack and start an Luck service within
	stack, err := node.New(&node.Config{DataDir: workspace, UseLightweightKDF: true, Name: testInstance})
	if err != nil {
		t.Fatalf("failed to create node: %v", err)
	}
	fortConf := &fort.Config{
	...
	if confOverride != nil {
		confOverride(fortConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return fort.New(ctx, fortConf) }); err != nil {
		t.Fatalf("failed to register Luck protocol: %v", err)
	}
