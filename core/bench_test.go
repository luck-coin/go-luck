...
// genTxRing returns a block generator that sends forter in a ring
...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	var db fortdb.Database
	if !disk {
		db = rawdb.NewMemoryDatabase()
	} else {
		dir, err := ioutil.TempDir("", "fort-core-bench")
		if err != nil {
...
// into a database.
func makeChainForBench(db fortdb.Database, full bool, count uint64) {
...
func benchWriteChain(b *testing.B, full bool, count uint64) {
	for i := 0; i < b.N; i++ {
		dir, err := ioutil.TempDir("", "fort-chain-bench")
		if err != nil {
			b.Fatalf("cannot create temporary directory: %v", err)
		}
