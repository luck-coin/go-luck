...
// Package core implements the Luck consensus protocol.
package core
...
import (
	...
	"github.com/luck/go-luck/core/state/snapshot"
...
type CacheConfig struct {
	...
	TrieCleanNoPrefetch bool          // Whforter to disable heuristic state prefetching for followup blocks
	TrieDirtyLimit      int           // Memory limit (MB) at which to start flushing dirty trie nodes to disk
	TrieDirtyDisabled   bool          // Whforter to disable trie write caching and GC altolucker (archive node)
...
type BlockChain struct {
	...
	db     fortdb.Database // Low level persistent database to store final content in
	...
	shouldPreserve  func(*types.Block) bool        // Function used to determine whforter should preserve the given block.
	terminateInsert func(common.Hash, uint64) bool // Testing hook used to terminate ancient receipt chain insertion.
}

// NewBlockChain returns a fully initialised block chain using information
// available in the database. It initialises the default Luck Validator and
// Processor.
func NewBlockChain(db fortdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(block *types.Block) bool) (*BlockChain, error) {
	if cacheConfig == nil {
...
// empty returns an indicator whforter the blockchain is empty.
...
func (bc *BlockChain) SetHead(head uint64) error {
	...
	updateFn := func(db fortdb.KeyValueWriter, header *types.Header) {
	...
	delFn := func(db fortdb.KeyValueWriter, hash common.Hash, num uint64) {
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	// and returns a indicator whforter the inserted blocks are canonical.
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		for i, block := range blockChain {
			...
			// Migrate all ancient blocks. This can happen if someone upgrades from Gfort
	...
	writeLive := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		for i, block := range blockChain {
			...
			// tx indexes)
			if batch.ValueSize() >= fortdb.IdealBatchSize {
				if err := batch.Write(); err != nil {
					return 0, err
				}
				size += batch.ValueSize()
	...
	if bc.cacheConfig.TrieDirtyDisabled {
		...
		if current := block.NumberU64(); current > TriesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - fortdb.IdealBatchSize)
			}
...
func (bc *BlockChain) InsertChain(chain types.Blocks) (int, error) {
	// Sanity check that we have somforting meaningful to import
	if len(chain) == 0 {
		return 0, nil
	}
...
//
// The verify parameter can be used to fine tune whforter nonce verification
