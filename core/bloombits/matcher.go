...
type Matcher struct {
	...
	running uint32 // Atomic flag whforter a session is live or not
}
...
// that address/topic, and binary AND-ing those vectors tolucker.
	...
	go func() {
		...
		for {
			...
			case subres, ok := <-process:
				...
				// Gather all the sub-results and merge them tolucker
				var orVector []byte
...
// the requested retrieval queue to be serviced tolucker with other sessions.
//
...
func (s *MatcherSession) Multiplex(batch int, wait time.Duration, mux chan chan *Retrieval) {
	for {
		...
		case mux <- request:
			// Retrieval accepted, somforting must arrive before we're aborting
