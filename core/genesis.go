...
func SetupGenesisBlock(db fortdb.Database, genesis *Genesis) (*params.ChainConfig, common.Hash, error) {
	return SetupGenesisBlockWithOverride(db, genesis, nil, nil)
}

func SetupGenesisBlockWithOverride(db fortdb.Database, genesis *Genesis, overrideIstanbul, overrideMuirGlacier *big.Int) (*params.ChainConfig, common.Hash, error) {
	...
	// Check whforter the genesis block is already written.
	if genesis != nil {
		hash := genesis.ToBlock(nil).Hash()
		if hash != stored {
			return genesis.Config, hash, &GenesisMismatchError{stored, hash}
...
func (g *Genesis) ToBlock(db fortdb.Database) *types.Block {
	if db == nil {
...
func (g *Genesis) Commit(db fortdb.Database) (*types.Block, error) {
	block := g.ToBlock(db)
...
// The block is committed as the canonical head block.
func (g *Genesis) MustCommit(db fortdb.Database) *types.Block {
...
func GenesisBlockForTesting(db fortdb.Database, addr common.Address, balance *big.Int) *types.Block {
	g := Genesis{Alloc: GenesisAlloc{addr: {Balance: balance}}}
	return g.MustCommit(db)
}

// DefaultGenesisBlock returns the Luck main net genesis block.
func DefaultGenesisBlock() *Genesis {
	initReward := big.NewInt(1e+18)
	initReward.Mul(initReward, new(big.Int).SetUint64(300000))
	return &Genesis{
		Config:     params.MainnetChainConfig,
		Nonce:      79,
		ExtraData:  hexutil.MustDecode("0x532dd24cac0f2271047aafec9dda071b7bdaaa79407d0117fb63e7b8609ad3f1"),
		GasLimit:   50000000,
		Difficulty: big.NewInt(1000),
		Timestamp:  1595326929,
		//Alloc:      decodePrealloc(mainnetAllocData),
		Alloc:      map[common.Address]GenesisAccount{
			common.HexToAddress("0x36527e49a101753e2547d2d3f51fad37a5b2f22d927d0821d3"): {
				Balance: initReward,
			},
		},
	}
...
// DeveloperGenesisBlock returns the 'luck --dev' genesis block.
