...
func ReadDatabaseVersion(db fortdb.KeyValueReader) *uint64 {
	var version uint64
...
func WriteDatabaseVersion(db fortdb.KeyValueWriter, version uint64) {
...
func ReadChainConfig(db fortdb.KeyValueReader, hash common.Hash) *params.ChainConfig {
...
func WriteChainConfig(db fortdb.KeyValueWriter, hash common.Hash, cfg *params.ChainConfig) {
	if cfg == nil {
		return
	}
...
func ReadPreimage(db fortdb.KeyValueReader, hash common.Hash) []byte {
	data, _ := db.Get(preimageKey(hash))
	return data
}

// WritePreimages writes the provided set of preimages to the database.
func WritePreimages(db fortdb.KeyValueWriter, preimages map[common.Hash][]byte) {
