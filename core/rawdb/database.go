...
import (
	...
	"github.com/luck/go-luck/fortdb/leveldb"
	"github.com/luck/go-luck/fortdb/memorydb"
...
func NewDatabase(db fortdb.KeyValueStore) fortdb.Database {
	return &nofreezedb{
		KeyValueStore: db,
	}
}
...
// storage.
func NewDatabaseWithFreezer(db fortdb.KeyValueStore, freezer string, namespace string) (fortdb.Database, error) {
	...
	if kvgenesis, _ := db.Get(headerHashKey(0)); len(kvgenesis) > 0 {
		if frozen, _ := frdb.Ancients(); frozen > 0 {
			// If the freezer already contains somforting, ensure that the genesis blocks
...
func NewMemoryDatabase() fortdb.Database {
...
func NewMemoryDatabaseWithCap(size int) fortdb.Database {
	return NewDatabase(memorydb.NewWithCap(size))
}

// NewLevelDBDatabase creates a persistent key-value database without a freezer
// moving immutable chain segments into cold storage.
func NewLevelDBDatabase(file string, cache int, handles int, namespace string) (fortdb.Database, error) {
...
// freezer moving immutable chain segments into cold storage.
func NewLevelDBDatabaseWithFreezer(file string, cache int, handles int, freezer string, namespace string) (fortdb.Database, error) {
...
func InspectDatabase(db fortdb.Database) error {
