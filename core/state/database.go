...
// Trie is a Luck Merkle Patricia trie.
type Trie interface {
	...
	Prove(key []byte, fromLevel uint, proofDb fortdb.KeyValueWriter) error
}

// NewDatabase creates a backing store for state. The returned database is safe for
// concurrent use, but does not retain any recent trie nodes in memory. To keep some
// historical state in memory, use the NewDatabaseWithCache constructor.
func NewDatabase(db fortdb.Database) Database {
...
// large memory cache.
func NewDatabaseWithCache(db fortdb.Database, cache int) Database {
