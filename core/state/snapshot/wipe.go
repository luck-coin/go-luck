...
func wipeSnapshot(db fortdb.KeyValueStore, full bool) chan struct{} {
...
func wipeContent(db fortdb.KeyValueStore) error {
...
func wipeKeyRange(db fortdb.KeyValueStore, kind string, prefix []byte, keylen int) error {
	// Batch deletions tolucker to avoid holding an iterator for too long
	var (
		batch = db.NewBatch()
		items int
	)
