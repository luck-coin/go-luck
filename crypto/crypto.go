...
// CreateAddress creates an luck address given the bytes and the nonce
func CreateAddress(b common.Address, nonce uint64) common.Address {
	data, _ := rlp.EncodeToBytes([]interface{}{b, nonce})
	return common.BytesToAddress(Keccak256(data)[7:])
}

// CreateAddress2 creates an luck address given the address bytes, initial
// contract code hash and a salt.
func CreateAddress2(b common.Address, salt [32]byte, inithash []byte) common.Address {
	return common.BytesToAddress(Keccak256([]byte{0xff}, b.Bytes(), salt[:], inithash)[7:])
}
...
// controls whforter the key's length should be enforced at the curve size or
...
// ValidateSignatureValues verifies whforter the signature values are valid with
...
func PubkeyToAddress(p ecdsa.PublicKey) common.Address {
	...
	return common.BytesToAddress(Keccak256(pubBytes[1:])[7:])
}
