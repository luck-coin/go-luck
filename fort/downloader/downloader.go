...
var (
	...
	maxQueuedHeaders         = 32 * 1024                    // [fort/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	stateDB    fortdb.Database  // Database to state sync into (and deduplicate via)
	...
	// Channels
	headerCh      chan dataPack        // [fort/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [fort/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [fort/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [fort/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [fort/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*types.Header // [fort/62] Channel to feed the header processor new tasks

	// for stateFetcher
	stateSyncStart chan *stateSync
	trackStateReq  chan *stateReq
	stateCh        chan dataPack // [fort/63] Channel receiving inbound node state data
...
func New(checkpoint uint64, stateDb fortdb.Database, stateBloom *trie.SyncBloom, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
// these are zero.
func (d *Downloader) Progress() luck.SyncProgress {
...
// Synchronising returns whforter the downloader is currently retrieving blocks.
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "fort", p.version, "head", hash, "td", td, "mode", d.mode)
...
func (d *Downloader) fetchHeight(p *peerConnection) (*types.Header, error) {
	...
	for {
		...
		case packet := <-d.headerCh:
			...
			// Make sure the peer actually gave somforting valid
	...
	for {
		...
		case packet := <-deliveryCh:
			...
			if peer := d.peers.Peer(packet.PeerId()); peer != nil {
				...
				// Unless a peer delivered somforting completely else than requested (usually
	...
	for {
		...
		case headers := <-d.headerProcCh:
			...
			if len(headers) == 0 {
				...
				// L: Request new headers up from 11 (R's TD was higher, it must have somforting)
				// R: Nothing to give
				if d.mode != LightSync {
				...
				// peer gave us somforting useful, we're already happy/progressed (above check).
			...
			gotHeaders = true
			for len(headers) > 0 {
				// Terminate if somforting failed in between processing chunks
				select {
				case <-d.cancelCh:
					return errCanceled
				default:
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	if index, err := d.blockchain.InsertChain(blocks); err != nil {
		if index < len(results) {
			...
			// The importer will put tolucker a new list of blocks to import, which is a superset
