...
type ProtocolManager struct {
	...
	fastSync  uint32 // Flag whforter fast sync is enabled (gets disabled if we already have blocks)
	acceptTxs uint32 // Flag whforter we're considered synchronised (enables transaction processing)
...
// NewProtocolManager returns a new Luck sub protocol manager. The Luck sub protocol manages peers capable
// with the Luck network.
func NewProtocolManager(config *params.ChainConfig, checkpoint *params.TrustedCheckpoint, mode downloader.SyncMode, networkID uint64, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb fortdb.Database, cacheLimit int, whitelist map[uint64]common.Hash) (*ProtocolManager, error) {
	...
	inserter := func(blocks types.Blocks) (int, error) {
		...
		// the propagated block if the head is too old. Unlucktely there is a corner
		...
		// accept each others' blocks until a restart. Unlucktely we haven't figured
		// out a way yet where nodes can decide unilaterally whforter the network is new
...
func (pm *ProtocolManager) makeProtocol(version uint) p2p.Protocol {
	...
	return p2p.Protocol{
		...
		PeerInfo: func(id lnode.ID) interface{} {
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Luck peer", "peer", id)

	// Unregister the peer from the downloader and Luck peer set
...
func (pm *ProtocolManager) Start(maxPeers int) {
	...
	go pm.txsyncLoop64() // TODO(karalabe): Legacy initial tx echange, drop with fort/64.
}
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Luck protocol stopped")
}
...
// handle is the callback invoked to manage the life cycle of an fort peer. When
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Luck peer connected", "name", p.Name())

	// Execute the Luck handshake
	var (
	...
	if err := p.Handshake(pm.networkID, td, hash, genesis.Hash(), forkid.NewID(pm.blockchain), pm.forkFilter); err != nil {
		p.Log().Debug("Luck handshake failed", "err", err)
		return err
	}

	// Register the peer locally
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Luck peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Luck message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case p.version >= fort63 && msg.Code == GetNodeDataMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= fort63 && msg.Code == NodeDataMsg:
	...
	case p.version >= fort63 && msg.Code == GetReceiptsMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= fort63 && msg.Code == ReceiptsMsg:
	...
	case msg.Code == NewPooledTransactionHashesMsg && p.version >= fort65:
	...
	case msg.Code == GetPooledTransactionsMsg && p.version >= fort65:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case msg.Code == TransactionMsg || (msg.Code == PooledTransactionsMsg && p.version >= fort65):
...
// NodeInfo represents a short summary of the Luck sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // Luck network ID (1=Frontier, 2=Morden, Ropsten=3, Testnet=4)
