...
// Tests that post fort protocol handshake, clients perform a mutual checkpoint
...
func testCheckpointChallenge(t *testing.T, syncmode downloader.SyncMode, checkpoint bool, timeout bool, empty bool, match bool, drop bool) {
	...
	peer, _ := newTestPeer("peer", fort63, pm, true)
	defer peer.close()
...
func testBroadcastBlock(t *testing.T, totalPeers, broadcastExpected int) {
	...
	for i := 0; i < totalPeers; i++ {
		peer, _ := newTestPeer(fmt.Sprintf("peer %d", i), fort63, pm, true)
		defer peer.close()
...
func TestBroadcastMalformedBlock(t *testing.T) {
	...
	// propagation
	source, _ := newTestPeer("source", fort63, pm, true)
	defer source.close()

	sink, _ := newTestPeer("sink", fort63, pm, true)
	defer sink.close()
