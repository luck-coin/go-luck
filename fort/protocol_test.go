...
func TestForkIDSplit(t *testing.T) {
	var (
		...
		fortNoFork, _  = NewProtocolManager(configNoFork, nil, downloader.FullSync, 1, new(event.TypeMux), &testTxPool{pool: make(map[common.Hash]*types.Transaction)}, engine, chainNoFork, dbNoFork, 1, nil)
		fortProFork, _ = NewProtocolManager(configProFork, nil, downloader.FullSync, 1, new(event.TypeMux), &testTxPool{pool: make(map[common.Hash]*types.Transaction)}, engine, chainProFork, dbProFork, 1, nil)
	)
	fortNoFork.Start(1000)
	fortProFork.Start(1000)

	// Both nodes should allow the other to connect (same genesis, next fork is the same)
	p2pNoFork, p2pProFork := p2p.MsgPipe()
	peerNoFork := newPeer(64, p2p.NewPeer(lnode.ID{1}, "", nil), p2pNoFork, nil)
	peerProFork := newPeer(64, p2p.NewPeer(lnode.ID{2}, "", nil), p2pProFork, nil)

	errc := make(chan error, 2)
	go func() { errc <- fortNoFork.handle(peerProFork) }()
	go func() { errc <- fortProFork.handle(peerNoFork) }()
...
func testSyncTransaction(t *testing.T, propagtion bool) {
	...
	go pmSender.handle(pmSender.newPeer(65, p2p.NewPeer(lnode.ID{}, "sender", nil), io2, pmSender.txpool.Get))
	go pmFetcher.handle(pmFetcher.newPeer(65, p2p.NewPeer(lnode.ID{}, "fetcher", nil), io1, pmFetcher.txpool.Get))
