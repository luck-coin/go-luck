...
func (pm *ProtocolManager) syncTransactions(p *peer) {
	...
	// The fort/65 protocol introduces proper transaction announcements, so instead
...
func (pm *ProtocolManager) txsyncLoop64() {
	...
	var (
		pending = make(map[lnode.ID]*txsync)
		sending = false               // whforter a send is active
	...
	send := func(s *txsync) {
		if s.p.version >= fort65 {
			panic("initial transaction syncer running on fort/65+")
		}
...
// nextSyncOp determines whforter sync is required at this time.
