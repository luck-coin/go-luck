...
func testFastSyncDisabling(t *testing.T, protocol int) {
	...
	io1, io2 := p2p.MsgPipe()
	go pmFull.handle(pmFull.newPeer(protocol, p2p.NewPeer(lnode.ID{}, "empty", nil), io2, pmFull.txpool.Get))
	go pmEmpty.handle(pmEmpty.newPeer(protocol, p2p.NewPeer(lnode.ID{}, "full", nil), io1, pmEmpty.txpool.Get))
