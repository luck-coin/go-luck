...
import (
	...
	"github.com/luck/go-luck/tests"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a Gfort console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = fort.getBlock(fort.getTransaction(tx).blockHash);
  var genesis = fort.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.fort.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  fort.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
