...
// Verify that Client implements the luck interfaces.
var (
	_ = luck.ChainReader(&Client{})
	_ = luck.TransactionReader(&Client{})
	_ = luck.ChainStateReader(&Client{})
	_ = luck.ChainSyncReader(&Client{})
	_ = luck.ContractCaller(&Client{})
	_ = luck.GasEstimator(&Client{})
	_ = luck.GasPricer(&Client{})
	_ = luck.LogFilterer(&Client{})
	_ = luck.PendingStateReader(&Client{})
	// _ = luck.PendingStateEventer(&Client{})
	_ = luck.PendingContractCaller(&Client{})
)
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	n.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		config := &fort.Config{Genesis: genesis}
		config.Ethash.PowMode = ethash.ModeFake
		fortservice, err = fort.New(ctx, config)
		return fortservice, err
	})
	...
	if _, err := fortservice.BlockChain().InsertChain(blocks[1:]); err != nil {
