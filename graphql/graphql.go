...
// Package graphql provides a GraphQL interface to Luck node data.
package graphql
...
// Account represents an Luck account at a particular block.
type Account struct {
	backend       fortapi.Backend
...
// Transaction represents an Luck transaction.
...
// Block represents an Luck block.
...
// `Log` objects.
func runFilter(ctx context.Context, be fortapi.Backend, filter *filters.Filter) ([]*Log, error) {
...
type CallData struct {
	From     *common.Address // The Luck address the call is from.
	To       *common.Address // The Luck address the call is to.
...
func (b *Block) Call(ctx context.Context, args struct {
	...
	result, gas, failed, err := fortapi.DoCall(ctx, b.backend, args.Data, *b.numberOrHash, nil, vm.Config{}, 5*time.Second, b.backend.RPCGasCap())
...
func (b *Block) EstimateGas(ctx context.Context, args struct {
	...
	gas, err := fortapi.DoEstimateGas(ctx, b.backend, args.Data, *b.numberOrHash, b.backend.RPCGasCap())
	return gas, err
}
...
func (p *Pending) Call(ctx context.Context, args struct {
	...
	pendingBlockNr := rpc.BlockNumberOrHashWithNumber(rpc.PendingBlockNumber)
	result, gas, failed, err := fortapi.DoCall(ctx, p.backend, args.Data, pendingBlockNr, nil, vm.Config{}, 5*time.Second, p.backend.RPCGasCap())
	status := hexutil.Uint64(1)
	if failed {
		status = 0
	}
...
func (p *Pending) EstimateGas(ctx context.Context, args struct {
	...
	pendingBlockNr := rpc.BlockNumberOrHashWithNumber(rpc.PendingBlockNumber)
	return fortapi.DoEstimateGas(ctx, p.backend, args.Data, pendingBlockNr, p.backend.RPCGasCap())
}
...
func (r *Resolver) SendRawTransaction(ctx context.Context, args struct{ Data hexutil.Bytes }) (common.Hash, error) {
	...
	hash, err := fortapi.SubmitTransaction(ctx, r.backend, tx)
	return hash, err
}
