...
import (
	...
	"github.com/luck/go-luck/accounts/scwallet"
...
// PublicLuckAPI provides an API to access Luck related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicLuckAPI struct {
	b Backend
}

// NewPublicLuckAPI creates a new Luck protocol API.
func NewPublicLuckAPI(b Backend) *PublicLuckAPI {
	return &PublicLuckAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicLuckAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
	price, err := s.b.SuggestPrice(ctx)
	return (*hexutil.Big)(price), err
}

// ProtocolVersion returns the current Luck protocol version this node supports
func (s *PublicLuckAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicLuckAPI) Syncing() (interface{}, error) {
...
// Sign calculates an Luck ECDSA signature for:
// keccack256("\x19Luck Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/luck/go-luck/wiki/Management-APIs#personal_sign
...
// Note, this function is compatible with fort_sign and personal_sign. As such it recovers
// the address of:
// hash = keccak256("\x19Luck Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://github.com/luck/go-luck/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid Luck signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and luck.
...
// PublicBlockChainAPI provides an API to access the Luck blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Luck blockchain API.
...
func RPCMarshalHeader(head *types.Header) map[string]interface{} {
	return map[string]interface{}{
		...
		"luck":				(*hexutil.Big)(head.Lucky),
		"basis":			(*hexutil.Big)(head.Basis),
		"firstNonce":		head.FirstNonce,
		"difficultyAlpha":  (*hexutil.Big)(head.DifficultyAlpha),
		"difficultyBeta":   (*hexutil.Big)(head.DifficultyBeta),
		"secondNonce":      head.SecondNonce,
	}
}
...
//
// https://github.com/luck/wiki/wiki/JSON-RPC#fort_sign
...
// PublicDebugAPI is the collection of Luck APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of Luck APIs exposed over the private
// debugging endpoint.
...
// Version returns the current luck protocol version.
