...
// similar requests at the maximum allowed rate and the test verifies whforter the
...
func getHead(ctx context.Context, t *testing.T, client *rpc.Client) (uint64, common.Hash) {
	...
	if err := client.CallContext(ctx, &res, "fort_getBlockByNumber", "latest", false); err != nil {
...
func testRequest(ctx context.Context, t *testing.T, client *rpc.Client) bool {
	...
	defer cancel()
	err := client.CallContext(c, &res, "fort_getBalance", addr, "latest")
	if err != nil {
...
func freezeClient(ctx context.Context, t *testing.T, server *rpc.Client, clientID lnode.ID) {
...
func setCapacity(ctx context.Context, t *testing.T, server *rpc.Client, clientID lnode.ID, cap uint64) {
	params := make(map[string]interface{})
	params["capacity"] = cap
	if err := server.CallContext(ctx, nil, "les_setClientParams", []lnode.ID{clientID}, []string{}, params); err != nil {
		t.Fatalf("Failed to set client capacity: %v", err)
	}
}

func getCapacity(ctx context.Context, t *testing.T, server *rpc.Client, clientID lnode.ID) uint64 {
	var res map[lnode.ID]map[string]interface{}
	if err := server.CallContext(ctx, &res, "les_clientInfo", []lnode.ID{clientID}, []string{}); err != nil {
...
func newLesServerService(ctx *adapters.ServiceContext) (node.Service, error) {
	...
	luck, err := fort.New(ctx.NodeContext, &config)
	if err != nil {
		return nil, err
	}
	server, err := NewLesServer(luck, &config)
	if err != nil {
		return nil, err
	}
	luck.AddLesServer(server)
	return luck, nil
}
