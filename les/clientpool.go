...
type clientPool struct {
	...
	stopCh     chan struct{}
	closed     bool
	removePeer func(lnode.ID)

	connectedMap   map[lnode.ID]*clientInfo
...
func newClientPool(db fortdb.Database, freeClientCap uint64, clock mclock.Clock, removePeer func(lnode.ID)) *clientPool {
	ndb := newNodeDB(db, clock)
	pool := &clientPool{
		ndb:            ndb,
		clock:          clock,
		connectedMap:   make(map[lnode.ID]*clientInfo),
...
func (f *clientPool) forClients(ids []lnode.ID, callback func(*clientInfo, lnode.ID) error) error {
	f.lock.Lock()
	defer f.lock.Unlock()
...
func (f *clientPool) balanceExhausted(id lnode.ID) {
	f.lock.Lock()
	defer f.lock.Unlock()
...
func (f *clientPool) getPosBalance(id lnode.ID) posBalance {
	f.lock.Lock()
	defer f.lock.Unlock()
...
func (f *clientPool) addBalance(id lnode.ID, amount int64, meta string) (uint64, uint64, error) {
	f.lock.Lock()
	defer f.lock.Unlock()
...
type nodeDB struct {
	...
	nbEvictCallBack func(mclock.AbsTime, negBalance) bool // Callback to determine whforter the negative balance can be evicted.
	clock           mclock.Clock
	closeCh         chan struct{}
	cleanupHook     func() // Test hook used for testing
}

func newNodeDB(db fortdb.Database, clock mclock.Clock) *nodeDB {
...
func (db *nodeDB) getOrNewPB(id lnode.ID) posBalance {
...
func (db *nodeDB) setPB(id lnode.ID, b posBalance) {
...
func (db *nodeDB) delPB(id lnode.ID) {
	key := db.key(id.Bytes(), false)
	db.db.Delete(key)
...
func (db *nodeDB) getPosBalanceIDs(start, stop lnode.ID, maxCount int) (result []lnode.ID) {
	if maxCount <= 0 {
		return
	}
...
// expireNodes iterates the whole node db and checks whforter the negative balance
// entry can deleted.
//
