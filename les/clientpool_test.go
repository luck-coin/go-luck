...
func (i poolTestPeer) ID() lnode.ID {
	return lnode.ID{byte(i % 256), byte(i >> 8)}
}
...
func TestPaidClientKickedOut(t *testing.T) {
	...
	removeFn := func(id lnode.ID) { kickedCh <- int(id[0]) }
	pool := newClientPool(db, 1, &clock, removeFn)
	defer pool.stop()
	pool.setLimits(10, uint64(10)) // Total capacity limit is 10
	pool.setDefaultFactors(priceFactors{1, 0, 1}, priceFactors{1, 0, 1})
...
func TestFreeClientKickedOut(t *testing.T) {
	...
	removeFn := func(id lnode.ID) { kicked <- int(id[0]) }
	pool := newClientPool(db, 1, &clock, removeFn)
	defer pool.stop()
	pool.setLimits(10, uint64(10)) // Total capacity limit is 10
	pool.setDefaultFactors(priceFactors{1, 0, 1}, priceFactors{1, 0, 1})
...
func TestPositiveBalanceCalculation(t *testing.T) {
	...
	removeFn := func(id lnode.ID) { kicked <- int(id[0]) } // Noop
	pool := newClientPool(db, 1, &clock, removeFn)
	defer pool.stop()
	pool.setLimits(10, uint64(10)) // Total capacity limit is 10
	pool.setDefaultFactors(priceFactors{1, 0, 1}, priceFactors{1, 0, 1})
...
func TestNodeDB(t *testing.T) {
	...
	var cases = []struct {
		...
		{lnode.ID{0x00, 0x01, 0x02}, "", posBalance{value: 100}, true},
		{lnode.ID{0x00, 0x01, 0x02}, "", posBalance{value: 200}, true},
		{lnode.ID{}, "127.0.0.1", negBalance{logValue: 10}, false},
		{lnode.ID{}, "127.0.0.1", negBalance{logValue: 20}, false},
	}
	for _, c := range cases {
		if c.positive {
