...
// parseNodeStr converts either an lnode address or a plain hex node id to lnode.ID
func parseNodeStr(nodeStr string) (lnode.ID, error) {
	if id, err := lnode.ParseID(nodeStr); err == nil {
		return id, nil
	}
	if node, err := lnode.Parse(lnode.ValidSchemes, nodeStr); err == nil {
		return node.ID(), nil
	} else {
		return lnode.ID{}, err
	}
