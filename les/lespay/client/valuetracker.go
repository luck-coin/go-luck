...
type ValueTracker struct {
	...
	db           fortdb.KeyValueStore
	connected    map[lnode.ID]*NodeValueTracker
	reqTypeCount int
...
func NewValueTracker(db fortdb.KeyValueStore, clock mclock.Clock, reqInfo []RequestInfo, updatePeriod time.Duration, transferRate, statsExpRate, offlineExpRate float64) *ValueTracker {
	now := clock.Now()
	...
	vt := &ValueTracker{
		clock:          clock,
		connected:      make(map[lnode.ID]*NodeValueTracker),
...
func (vt *ValueTracker) Register(id lnode.ID) *NodeValueTracker {
	vt.lock.Lock()
...
func (vt *ValueTracker) Unregister(id lnode.ID) {
	vt.lock.Lock()
	defer vt.lock.Unlock()
...
func (vt *ValueTracker) GetNode(id lnode.ID) *NodeValueTracker {
	vt.lock.Lock()
	defer vt.lock.Unlock()
...
// then a new node is created.
func (vt *ValueTracker) loadOrNewNode(id lnode.ID) *NodeValueTracker {
...
func (vt *ValueTracker) saveNode(id lnode.ID, nv *NodeValueTracker) {
	recentRtStats := nv.rtStats
	recentRtStats.SubStats(&nv.lastRtStats)
