...
// peerIdToString converts lnode.ID to a string form
func peerIdToString(id lnode.ID) string {
...
type peerCommons struct {
	*p2p.Peer
	...
	frozen       uint32    // Flag whforter the peer is frozen.
...
// canQueue returns an indicator whforter the peer can queue a operation.
...
func (p *peerCommons) mustQueueSend(f func()) {
	for {
		// Check whforter the stopCh is closed.
		select {
		case <-p.closeCh:
			return
		default:
...
func (p *peerCommons) Info() *fort.PeerInfo {
	return &fort.PeerInfo{
		Version:    p.version,
		Difficulty: p.Td(),
...
type serverPeer struct {
	...
	// Status fields
	trusted                 bool   // The flag whforter the server is selected as trusted server.
	onlyAnnounce            bool   // The flag whforter the server sends announcement only.
	...
	// Callbacks
	hasBlock func(common.Hash, uint64, bool) bool // Used to determine whforter the server has the specified block.
}
	...
	return p.handshake(td, head, headNum, genesis, func(lists *keyValueList) {
		...
		if !server.config.UltraLightOnlyAnnounce {
			...
			*lists = (*lists).add("serveStateSince", uint64(0))
			...
			// If local luck node is running in archive mode, advertise ourselves we have
...
// participating in the Light Luck sub-protocol.
