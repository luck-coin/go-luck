...
type LesServer struct {
	...
	archiveMode bool // Flag whforter the luck node runs in archive mode.
	peers       *clientPeerSet
...
func NewLesServer(e *fort.Luck, config *fort.Config) (*LesServer, error) {
	...
	srv.clientPool = newClientPool(srv.chainDb, srv.freeCapacity, mclock.System{}, func(id lnode.ID) { go srv.peers.unregister(peerIdToString(id)) })
...
func (s *LesServer) Protocols() []p2p.Protocol {
	ps := s.makeProtocols(ServerProtocolVersions, s.handler.runPeer, func(id lnode.ID) interface{} {
