...
const (
	...
	fortVersion        = 63              // equivalent fort version for the downloader
...
type serverHandler struct {
	...
	synced  func() bool    // Callback function used to determine whforter local node is synced.

	// Testing fields
	addTxsSync bool
}

func newServerHandler(server *LesServer, blockchain *core.BlockChain, chainDb fortdb.Database, txpool *core.TxPool, synced func() bool) *serverHandler {
...
func (h *serverHandler) handle(p *clientPeer) error {
	p.Log().Debug("Light Luck peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, h.blockchain.Genesis().Hash(), h.server); err != nil {
		p.Log().Debug("Light Luck handshake failed", "err", err)
		return err
	}
	if p.server {
	...
	if !h.server.clientPool.connect(p, 0) {
		p.Log().Debug("Light Luck peer registration failed", "err", errFullClientPool)
		return errFullClientPool
	}
	// Register the peer locally
	if err := h.server.peers.register(p); err != nil {
		h.server.clientPool.disconnect(p)
		p.Log().Error("Light Luck peer registration failed", "err", err)
		return err
	}
	...
	for {
		select {
		case err := <-p.errCh:
			p.Log().Debug("Failed to send light luck response", "err", err)
			return err
		default:
		}
		if err := h.handleMsg(p, &wg); err != nil {
			p.Log().Debug("Light Luck message handling failed", "err", err)
			return err
		}
	}
}
...
func (h *serverHandler) handleMsg(p *clientPeer, wg *sync.WaitGroup) error {
	...
	p.Log().Trace("Light Luck message arrived", "code", msg.Code, "bytes", msg.Size)
	...
	// accept returns an indicator whforter the request can be served.
