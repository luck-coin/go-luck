...
import (
	...
	"github.com/luck/go-luck/les/utils"
...
type serverPool struct {
	...
	discNodes     chan *lnode.Node
	discLookups   chan bool

	trustedNodes         map[lnode.ID]*lnode.Node
	entries              map[lnode.ID]*poolEntry
...
func newServerPool(db fortdb.Database, ulcServers []string) *serverPool {
	pool := &serverPool{
		db:           db,
		entries:      make(map[lnode.ID]*poolEntry),
...
func (pool *serverPool) start(server *p2p.Server, topic discv5.Topic) {
	...
	if pool.server.DiscV5 != nil {
		...
		pool.discNodes = make(chan *lnode.Node, 100)
...
// discoverNodes wraps SearchTopic, converting result nodes to lnode.Node.
func (pool *serverPool) discoverNodes() {
	...
	for n := range ch {
		...
		pool.discNodes <- lnode.NewV4(pubkey, n.IP, int(n.TCP), int(n.UDP))
	}
}
...
func (pool *serverPool) connect(p *serverPeer, node *lnode.Node) *poolEntry {
	log.Debug("Connect new entry", "lnode", p.id)
...
func (pool *serverPool) registered(entry *poolEntry) {
	log.Debug("Registered new entry", "lnode", entry.node.ID())
...
func (pool *serverPool) disconnect(entry *poolEntry) {
	...
	log.Debug("Disconnected old entry", "lnode", entry.node.ID())
...
func (pool *serverPool) findOrNewNode(node *lnode.Node) *poolEntry {
	now := mclock.Now()
...
// parseTrustedNodes returns valid and parsed lnodes
func parseTrustedNodes(trustedNodes []string) map[lnode.ID]*lnode.Node {
	nodes := make(map[lnode.ID]*lnode.Node)

	for _, node := range trustedNodes {
		node, err := lnode.Parse(lnode.ValidSchemes, node)
		if err != nil {
			log.Warn("Trusted node URL invalid", "lnode", node, "err", err)
			continue
		}
		nodes[node.ID()] = node
	}
...
func (e *poolEntry) DecodeRLP(s *rlp.Stream) error {
	...
	e.node = lnode.NewV4(pubkey, entry.IP, int(entry.Port), int(entry.Port))
