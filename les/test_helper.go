...
import (
	...
	"github.com/luck/go-luck/accounts/abi/bind/backends"
	"github.com/luck/go-luck/common"
	"github.com/luck/go-luck/common/mclock"
	"github.com/luck/go-luck/consensus/ethash"
	"github.com/luck/go-luck/contracts/checkpointoracle/contract"
...
func prepare(n int, backend *backends.SimulatedBackend) {
	...
	for i := 0; i < n; i++ {
		...
		case 0:
			...
			// bankUser transfers some forter to user1
		...
		case 1:
			...
			// bankUser transfers more forter to user1
		...
		case 2:
			// bankUser transfer some forter to signer
			bankNonce, _ := backend.PendingNonceAt(ctx, bankAddr)
...
func testIndexers(db fortdb.Database, odr light.OdrBackend, config *light.IndexerConfig) []*core.ChainIndexer {
	var indexers [3]*core.ChainIndexer
	indexers[0] = light.NewChtIndexer(db, odr, config.ChtSize, config.ChtConfirms)
	indexers[1] = fort.NewBloomIndexer(db, config.BloomSize, config.BloomConfirms)
...
func newTestClientHandler(backend *backends.SimulatedBackend, odr *LesOdr, indexers []*core.ChainIndexer, db fortdb.Database, peers *serverPeerSet, ulcServers []string, ulcFraction int) *clientHandler {
	var (
	...
	client := &LightLuck{
		lesCommons: lesCommons{
			...
			config:      &fort.Config{LightPeers: 100, NetworkId: NetworkId},
...
func newTestServerHandler(blocks int, indexers []*core.ChainIndexer, db fortdb.Database, peers *clientPeerSet, clock mclock.Clock) (*serverHandler, *backends.SimulatedBackend) {
	var (
		gspec = core.Genesis{
			Config:   params.AllEthashProtocolChanges,
			Alloc:    core.GenesisAlloc{bankAddr: {Balance: bankFunds}},
