...
import (
	...
	"github.com/luck/go-luck/common/bitutil"
...
func GetChtRoot(db fortdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
...
func StoreChtRoot(db fortdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
type ChtIndexerBackend struct {
	diskdb, trieTable    fortdb.Database
	odr                  OdrBackend
...
func NewChtIndexer(db fortdb.Database, odr OdrBackend, size, confirms uint64) *core.ChainIndexer {
...
func GetBloomTrieRoot(db fortdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db fortdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func NewBloomTrieIndexer(db fortdb.Database, odr OdrBackend, parentSize, size uint64) *core.ChainIndexer {
