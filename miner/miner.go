...
// Package miner implements Luck block creation and mining.
package miner
...
type Miner struct {
	...
	canStart    int32 // can start indicates whforter we can start the mining operation
	shouldStart int32 // should start indicates whforter we should start after sync
}

func New(fort Backend, config *Config, chainConfig *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine, isLocalBlock func(block *types.Block) bool) *Miner {
	miner := &Miner{
		fort:      fort,
		mux:      mux,
		engine:   engine,
		exitCh:   make(chan struct{}),
		worker:   newWorker(config, chainConfig, engine, fort, mux, isLocalBlock, true),
		canStart: 1,
	}
	go miner.update()
