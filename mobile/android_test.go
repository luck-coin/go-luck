...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gfort.LightScryptN, Gfort.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.luck", new NodeConfig());
			node.start();
			...
			LuckClient ec = node.getLuckClient();
	...
	// signatures too. Regression test for go-luck issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altolucker
	...
	workspace, err := ioutil.TempDir("", "luck-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/luck/lucktest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gfort and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.luck", "github.com/luck/go-luck/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "luck.aar"), "luck.aar")

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "luck", "lucktest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
