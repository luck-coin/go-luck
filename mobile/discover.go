...
// Contains all the wrappers from the accounts package to support client side lnode
...
//
//    lnode://<hex node id>@10.3.58.6:30303?discport=30301
func NewEnode(rawurl string) (lnode *Enode, _ error) {
...
// NewEnodes creates a slice of uninitialized lnodes.
...
// Size returns the number of lnodes in the slice.
func (e *Enodes) Size() int {
	return len(e.nodes)
}

// Get returns the lnode at the given index from the slice.
func (e *Enodes) Get(index int) (lnode *Enode, _ error) {
...
// Set sets the lnode at the given index in the slice.
func (e *Enodes) Set(index int, lnode *Enode) error {
	if index < 0 || index >= len(e.nodes) {
		return errors.New("index out of bounds")
	}
	e.nodes[index] = lnode.node
	return nil
}

// Append adds a new lnode element to the end of the slice.
func (e *Enodes) Append(lnode *Enode) {
	e.nodes = append(e.nodes, lnode.node)
}
