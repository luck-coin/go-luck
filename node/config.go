...
import (
	...
	"github.com/luck/go-luck/accounts/external"
	"github.com/luck/go-luck/accounts/keystore"
	"github.com/luck/go-luck/accounts/scwallet"
	"github.com/luck/go-luck/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of luck is "luck". If no
...
// ExtRPCEnabled returns the indicator whforter node enables the external
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gfort", keep that.
	if name == "luck" || name == "luck-testnet" {
		name = "Luck"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "luck" instances.
var isOldGfortResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by luck 1.4 are used if they exist.
	if warn, isOld := isOldGfortResource[path]; isOld {
		oldpath := ""
		if c.name() == "luck" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGfortResourceWarning, "Using deprecated resource file %s, please move this file to the 'luck' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
// StaticNodes returns a list of node lnode URLs configured as static nodes.
func (c *Config) StaticNodes() []*lnode.Node {
	return c.parsePersistentNodes(&c.staticNodesWarning, c.ResolvePath(datadirStaticNodes))
}

// TrustedNodes returns a list of node lnode URLs configured as trusted nodes.
func (c *Config) TrustedNodes() []*lnode.Node {
	return c.parsePersistentNodes(&c.trustedNodesWarning, c.ResolvePath(datadirTrustedNodes))
}

// parsePersistentNodes parses a list of discovery node URLs loaded from a .json
// file from within the data directory.
func (c *Config) parsePersistentNodes(w *bool, path string) []*lnode.Node {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-luck-keystore")
		ephemeral = keydir
	}
