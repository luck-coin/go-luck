...
func (n *Node) OpenDatabase(name string, cache, handles int, namespace string) (fortdb.Database, error) {
	if n.config.DataDir == "" {
...
func (n *Node) OpenDatabaseWithFreezer(name string, cache, handles int, freezer, namespace string) (fortdb.Database, error) {
	if n.config.DataDir == "" {
		return rawdb.NewMemoryDatabase(), nil
	}
