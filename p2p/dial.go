...
type NodeDialer interface {
	Dial(context.Context, *lnode.Node) (net.Conn, error)
}

type nodeResolver interface {
	Resolve(*lnode.Node) *lnode.Node
}
...
func (t tcpDialer) Dial(ctx context.Context, dest *lnode.Node) (net.Conn, error) {
	return t.d.DialContext(ctx, "tcp", nodeAddr(dest).String())
}

func nodeAddr(n *lnode.Node) net.Addr {
...
type dialScheduler struct {
	...
	ctx         context.Context
	nodesIn     chan *lnode.Node
	doneCh      chan *dialTask
	addStaticCh chan *lnode.Node
	remStaticCh chan *lnode.Node
	addPeerCh   chan *conn
	remPeerCh   chan *conn

	// Everything below here belongs to loop and
	// should only be accessed by code on the loop goroutine.
	dialing   map[lnode.ID]*dialTask // active tasks
	peers     map[lnode.ID]connFlag  // all connected peers
...
type dialSetupFunc func(net.Conn, connFlag, *lnode.Node) error

type dialConfig struct {
	self           lnode.ID         // our own ID
...
func newDialScheduler(config dialConfig, it lnode.Iterator, setupFunc dialSetupFunc) *dialScheduler {
	d := &dialScheduler{
		dialConfig:  config.withDefaults(),
		setupFunc:   setupFunc,
		dialing:     make(map[lnode.ID]*dialTask),
		static:      make(map[lnode.ID]*dialTask),
		peers:       make(map[lnode.ID]connFlag),
		doneCh:      make(chan *dialTask),
		nodesIn:     make(chan *lnode.Node),
		addStaticCh: make(chan *lnode.Node),
		remStaticCh: make(chan *lnode.Node),
...
func (d *dialScheduler) addStatic(n *lnode.Node) {
	select {
...
func (d *dialScheduler) removeStatic(n *lnode.Node) {
	select {
...
func (d *dialScheduler) loop(it lnode.Iterator) {
	var (
		nodesCh    chan *lnode.Node
...
func (d *dialScheduler) readNodes(it lnode.Iterator) {
	defer d.wg.Done()
...
func (d *dialScheduler) checkDial(n *lnode.Node) error {
	if n.ID() == d.self {
		return errSelf
	}
...
func (d *dialScheduler) updateStaticPool(id lnode.ID) {
	task, ok := d.static[id]
...
func newDialTask(dest *lnode.Node, flags connFlag) *dialTask {
...
func (t *dialTask) dial(d *dialScheduler, dest *lnode.Node) error {
