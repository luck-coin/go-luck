...
func TestDialSchedStaticDial(t *testing.T) {
	...
	runDialTest(t, config, []dialTestRound{
		...
		// two dials are launched to attempt to fill it.
		{
			...
			wantResolves: map[lnode.ID]*lnode.Node{
				uintID(0x04): nil,
				uintID(0x05): nil,
				uintID(0x06): nil,
			},
...
type dialTestRound struct {
	...
	discovered   []*lnode.Node        // newly discovered nodes
	succeeded    []lnode.ID           // dials which succeed this round
	failed       []lnode.ID           // dials which fail this round
	wantResolves map[lnode.ID]*lnode.Node
	wantNewDials []*lnode.Node // dials that should be launched in this round
}
...
func runDialTest(t *testing.T, config dialConfig, rounds []dialTestRound) {
	...
	setup := func(fd net.Conn, f connFlag, node *lnode.Node) error {
...
func (it *dialTestIterator) addNodes(nodes []*lnode.Node) {
	it.mu.Lock()
	defer it.mu.Unlock()
...
func (it *dialTestIterator) Node() *lnode.Node {
	return it.cur
}
...
type dialTestDialer struct {
	...
	blocked map[lnode.ID]*dialTestReq
}
...
func newDialTestDialer() *dialTestDialer {
	return &dialTestDialer{
		...
		blocked: make(map[lnode.ID]*dialTestReq),
	}
}

// Dial implements NodeDialer.
func (d *dialTestDialer) Dial(ctx context.Context, n *lnode.Node) (net.Conn, error) {
...
func (d *dialTestDialer) waitForDials(nodes []*lnode.Node) error {
	waitset := make(map[lnode.ID]*lnode.Node)
	for _, n := range nodes {
		waitset[n.ID()] = n
	}
...
func (d *dialTestDialer) completeDials(ids []lnode.ID, err error) error {
	for _, id := range ids {
		req := d.blocked[id]
		if req == nil {
...
type dialTestResolver struct {
	mu      sync.Mutex
	calls   []lnode.ID
	answers map[lnode.ID]*lnode.Node
}

func (t *dialTestResolver) setAnswers(m map[lnode.ID]*lnode.Node) {
	t.mu.Lock()
	defer t.mu.Unlock()
...
func (t *dialTestResolver) Resolve(n *lnode.Node) *lnode.Node {
	t.mu.Lock()
	defer t.mu.Unlock()
