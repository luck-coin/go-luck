...
func newLookup(ctx context.Context, tab *Table, target lnode.ID, q queryFunc) *lookup {
	it := &lookup{
		tab:       tab,
		queryfunc: q,
		asked:     make(map[lnode.ID]bool),
...
func (it *lookupIterator) Node() *lnode.Node {
	if len(it.buffer) == 0 {
		return nil
	}
