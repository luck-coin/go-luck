...
func (e encPubkey) id() lnode.ID {
	return lnode.ID(crypto.Keccak256Hash(e[:]))
}
...
func wrapNode(n *lnode.Node) *node {
	return &node{Node: *n}
}

func wrapNodes(ns []*lnode.Node) []*node {
...
func unwrapNode(n *node) *lnode.Node {
	return &n.Node
}

func unwrapNodes(ns []*node) []*lnode.Node {
	result := make([]*lnode.Node, len(ns))
	for i, n := range ns {
