...
type Table struct {
	...
	log        log.Logger
	db         *lnode.DB // database of known nodes
	net        transport
...
type transport interface {
	Self() *lnode.Node
	RequestENR(*lnode.Node) (*lnode.Node, error)
	lookupRandom() []*lnode.Node
	lookupSelf() []*lnode.Node
	ping(*lnode.Node) (seq uint64, err error)
}
...
func newTable(t transport, db *lnode.DB, bootnodes []*lnode.Node, log log.Logger) (*Table, error) {
	tab := &Table{
		net:        t,
		db:         db,
...
func (tab *Table) self() *lnode.Node {
	return tab.net.Self()
}
...
func (tab *Table) ReadRandomNodes(buf []*lnode.Node) (n int) {
	if !tab.isInitDone() {
		return 0
	}
	tab.mutex.Lock()
...
func (tab *Table) getNode(id lnode.ID) *lnode.Node {
	tab.mutex.Lock()
	defer tab.mutex.Unlock()
...
func (tab *Table) setFallbackNodes(nodes []*lnode.Node) error {
	for _, n := range nodes {
...
// isInitDone returns whforter the table's initial seeding procedure has completed.
...
func (tab *Table) closest(target lnode.ID, nresults int, checklive bool) *nodesByDistance {
...
func (tab *Table) bucket(id lnode.ID) *bucket {
	d := lnode.LogDist(tab.self().ID(), id)
...
func contains(ns []*node, id lnode.ID) bool {
	for _, n := range ns {
		if n.ID() == id {
			return true
		}
...
func (h *nodesByDistance) push(n *node, maxElems int) {
	ix := sort.Search(len(h.entries), func(i int) bool {
		return lnode.DistCmp(h.target, h.entries[i].ID(), n.ID()) > 0
	})
