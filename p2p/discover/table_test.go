...
func testPingReplace(t *testing.T, newNodeIsResponding, lastInBucketIsResponding bool) {
	...
	pingSender := wrapNode(lnode.NewV4(&pingKey.PublicKey, net.IP{}, 99, 99))
	...
	if !transport.pinged[last.ID()] {
		// Oldest node in bucket is pinged to see whforter it is still alive.
...
func TestBucket_bumpNoDuplicates(t *testing.T) {
	...
	cfg := &quick.Config{
		...
		Values: func(args []reflect.Value, rand *rand.Rand) {
			...
			nodes := make([]*node, n)
			for i := range nodes {
				nodes[i] = nodeAtDistance(lnode.ID{}, 200, intIP(200))
			}
	...
	test := func(test *closeTest) bool {
		...
		for _, b := range tab.buckets {
			for _, n := range b.entries {
				...
				if lnode.DistCmp(test.Target, n.ID(), farthestResult) < 0 {
...
func TestTable_ReadRandomNodesGetAll(t *testing.T) {
	cfg := &quick.Config{
		...
		Rand:     rand.New(rand.NewSource(time.Now().Unix())),
		Values: func(args []reflect.Value, rand *rand.Rand) {
			args[0] = reflect.ValueOf(make([]*lnode.Node, rand.Intn(1000)))
		},
	}
	test := func(buf []*lnode.Node) bool {
		transport := newPingRecorder()
		tab, db := newTestTable(transport)
		defer db.Close()
		defer tab.close()
...
func (*closeTest) Generate(rand *rand.Rand, size int) reflect.Value {
	t := &closeTest{
		Self:   gen(lnode.ID{}, rand).(lnode.ID),
		Target: gen(lnode.ID{}, rand).(lnode.ID),
		N:      rand.Intn(bucketSize),
	}
	for _, id := range gen([]lnode.ID{}, rand).([]lnode.ID) {
		r := new(enr.Record)
		r.Set(enr.IP(genIP(rand)))
		n := wrapNode(lnode.SignNull(r, id))
		n.livenessChecks = 1
...
func TestTable_addVerifiedNode(t *testing.T) {
	...
	newn2 := wrapNode(lnode.SignNull(newrec, n2.ID()))
...
func TestTable_revalidateSyncRecord(t *testing.T) {
	...
	id := lnode.ID{1}
	n1 := wrapNode(lnode.SignNull(&r, id))
	tab.addSeenNode(n1)

	// Update the node record.
	r.Set(enr.WithEntry("foo", "bar"))
	n2 := lnode.SignNull(&r, id)
