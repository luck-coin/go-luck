...
func TestUDPv4_Lookup(t *testing.T) {
	...
	for _, e := range results {
		t.Logf("  ld=%d, %x", lnode.LogDist(lookupTestnet.target.id(), e.ID()), e.ID().Bytes())
	}
...
func TestUDPv4_LookupIterator(t *testing.T) {
	...
	seen := make(map[lnode.ID]*lnode.Node)
	...
	results := make([]*lnode.Node, 0, len(seen))
	for _, n := range seen {
...
func serveTestnet(test *udpTest, testnet *preminedTestnet) {
	for done := false; !done; {
			...
			case *findnodeV4:
				dist := lnode.LogDist(n.ID(), testnet.target.id())
...
func checkLookupResults(t *testing.T, tn *preminedTestnet, results []*lnode.Node) {
	t.Helper()
	t.Logf("results:")
	for _, e := range results {
		t.Logf("  ld=%d, %x", lnode.LogDist(tn.target.id(), e.ID()), e.ID().Bytes())
	}
...
func (tn *preminedTestnet) nodes() []*lnode.Node {
	result := make([]*lnode.Node, 0, tn.len())
...
func (tn *preminedTestnet) node(dist, index int) *lnode.Node {
	key := tn.dists[dist][index]
	rec := new(enr.Record)
	rec.Set(enr.IP{127, byte(dist >> 8), byte(dist), byte(index)})
	rec.Set(enr.UDP(5000))
	lnode.SignV4(rec, key)
	n, _ := lnode.New(lnode.ValidSchemes, rec)
	return n
}

func (tn *preminedTestnet) nodeByAddr(addr *net.UDPAddr) (*lnode.Node, *ecdsa.PrivateKey) {
...
func (tn *preminedTestnet) neighborsAtDistance(base *lnode.Node, distance uint, elems int) []*lnode.Node {
	nodes := nodesByDistance{target: base.ID()}
	for d := range lookupTestnet.dists {
		for i := range lookupTestnet.dists[d] {
			n := lookupTestnet.node(d, i)
			if uint(lnode.LogDist(n.ID(), base.ID())) == distance {
...
func (tn *preminedTestnet) closest(n int) (nodes []*lnode.Node) {
	...
	sort.Slice(nodes, func(i, j int) bool {
		return lnode.DistCmp(tn.target.id(), nodes[i].ID(), nodes[j].ID()) < 0
	})
	return nodes[:n]
}
...
func (tn *preminedTestnet) mine() {
	...
	for found < need {
		k := newkey()
		ld := lnode.LogDist(targetSha, encodePubkey(&k.PublicKey).id())
