...
type packetV4 interface {
	// preverify checks whforter the packet is valid and should be handled at all.
	preverify(t *UDPv4, from *net.UDPAddr, fromID lnode.ID, fromKey encPubkey) error
	// handle handles the packet.
	handle(t *UDPv4, from *net.UDPAddr, fromID lnode.ID, mac []byte)
...
func (t *UDPv4) nodeFromRPC(sender *net.UDPAddr, rn rpcNode) (*node, error) {
	...
	n := wrapNode(lnode.NewV4(key, rn.IP, int(rn.TCP), int(rn.UDP)))
...
func nodeToRPC(n *node) rpcNode {
	...
	var ekey encPubkey
	if err := n.Load((*lnode.Secp256k1)(&key)); err == nil {
...
type replyMatcher struct {
	...
	// reply was acceptable. The second return value indicates whforter the callback should
...
type reply struct {
	...
	ip   net.IP
	data packetV4
	// loop indicates whforter there was
	// a matching request by sending on this channel.
	matched chan<- bool
}

func ListenV4(c UDPConn, ln *lnode.LocalNode, cfg Config) (*UDPv4, error) {
...
func (t *UDPv4) Resolve(n *lnode.Node) *lnode.Node {
...
func (t *UDPv4) Ping(n *lnode.Node) error {
	_, err := t.ping(n)
	return err
}

// ping sends a ping message to the given node and waits for a reply.
func (t *UDPv4) ping(n *lnode.Node) (seq uint64, err error) {
...
func (t *UDPv4) sendPing(toid lnode.ID, toaddr *net.UDPAddr, callback func()) *replyMatcher {
...
func (t *UDPv4) LookupPubkey(key *ecdsa.PublicKey) []*lnode.Node {
	if t.tab.len() == 0 {
...
func (t *UDPv4) RandomNodes() lnode.Iterator {
	return newLookupIterator(t.closeCtx, t.newRandomLookup)
}

// lookupRandom implements transport.
func (t *UDPv4) lookupRandom() []*lnode.Node {
	return t.newRandomLookup(t.closeCtx).run()
}

// lookupSelf implements transport.
func (t *UDPv4) lookupSelf() []*lnode.Node {
...
func (t *UDPv4) newLookup(ctx context.Context, targetKey encPubkey) *lookup {
	target := lnode.ID(crypto.Keccak256Hash(targetKey[:]))
...
func (t *UDPv4) findnode(toid lnode.ID, toaddr *net.UDPAddr, target encPubkey) ([]*node, error) {
...
func (t *UDPv4) RequestENR(n *lnode.Node) (*lnode.Node, error) {
	...
	respN, err := lnode.New(lnode.ValidSchemes, &rm.reply.(*enrResponseV4).Record)
	if err != nil {
		return nil, err
	}
	if respN.ID() != n.ID() {
...
func (t *UDPv4) pending(id lnode.ID, ip net.IP, ptype byte, callback replyMatchFunc) *replyMatcher {
...
// whforter any matcher considered the packet acceptable.
func (t *UDPv4) handleReply(from lnode.ID, fromIP net.IP, req packetV4) bool {
...
func (t *UDPv4) loop() {
	...
	for {
		...
		case r := <-t.gotreply:
			var matched bool // whforter any replyMatcher considered the reply acceptable.
			for el := plist.Front(); el != nil; el = el.Next() {
...
func (t *UDPv4) send(toaddr *net.UDPAddr, toid lnode.ID, req packetV4) ([]byte, error) {
	packet, hash, err := t.encode(t.priv, req)
	if err != nil {
		return hash, err
	}
...
func (t *UDPv4) write(toaddr *net.UDPAddr, toid lnode.ID, what string, packet []byte) error {
...
func (t *UDPv4) checkBond(id lnode.ID, ip net.IP) bool {
	return time.Since(t.db.LastPongReceived(id, ip)) < bondExpiration
}

// ensureBond solicits a ping from a node if we haven't seen a ping from it for a while.
// This ensures there is a valid endpoint proof on the remote end.
func (t *UDPv4) ensureBond(toid lnode.ID, toaddr *net.UDPAddr) {
...
// expired checks whforter the given UNIX time stamp is in the past.
...
func (req *pingV4) preverify(t *UDPv4, from *net.UDPAddr, fromID lnode.ID, fromKey encPubkey) error {
...
func (req *pingV4) handle(t *UDPv4, from *net.UDPAddr, fromID lnode.ID, mac []byte) {
	// Reply.
	seq, _ := rlp.EncodeToBytes(t.localNode.Node().Seq())
	...
	n := wrapNode(lnode.NewV4(req.senderKey, from.IP, int(req.From.TCP), from.Port))
...
func (req *pongV4) preverify(t *UDPv4, from *net.UDPAddr, fromID lnode.ID, fromKey encPubkey) error {
	if expired(req.Expiration) {
		return errExpired
	}
...
func (req *pongV4) handle(t *UDPv4, from *net.UDPAddr, fromID lnode.ID, mac []byte) {
	t.localNode.UDPEndpointStatement(from, &net.UDPAddr{IP: req.To.IP, Port: int(req.To.UDP)})
...
func (req *findnodeV4) preverify(t *UDPv4, from *net.UDPAddr, fromID lnode.ID, fromKey encPubkey) error {
	if expired(req.Expiration) {
		return errExpired
	}
...
func (req *findnodeV4) handle(t *UDPv4, from *net.UDPAddr, fromID lnode.ID, mac []byte) {
	// Determine closest nodes.
	target := lnode.ID(crypto.Keccak256Hash(req.Target[:]))
	t.tab.mutex.Lock()
...
func (req *neighborsV4) preverify(t *UDPv4, from *net.UDPAddr, fromID lnode.ID, fromKey encPubkey) error {
	if expired(req.Expiration) {
		return errExpired
	}
	if !t.handleReply(fromID, from.IP, req) {
...
func (req *neighborsV4) handle(t *UDPv4, from *net.UDPAddr, fromID lnode.ID, mac []byte) {
}
...
func (req *enrRequestV4) preverify(t *UDPv4, from *net.UDPAddr, fromID lnode.ID, fromKey encPubkey) error {
	if expired(req.Expiration) {
		return errExpired
	}
	if !t.checkBond(fromID, from.IP) {
...
func (req *enrRequestV4) handle(t *UDPv4, from *net.UDPAddr, fromID lnode.ID, mac []byte) {
...
func (req *enrResponseV4) preverify(t *UDPv4, from *net.UDPAddr, fromID lnode.ID, fromKey encPubkey) error {
	if !t.handleReply(fromID, from.IP, req) {
		return errUnsolicitedReply
	}
	return nil
...
func (req *enrResponseV4) handle(t *UDPv4, from *net.UDPAddr, fromID lnode.ID, mac []byte) {
}
