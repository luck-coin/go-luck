...
func TestClientSyncTree(t *testing.T) {
	r := mapResolver{
		...
		"C7HRFPF3BLGF3YR4DY5KX3SMBE.n": "enrtree://AM5FCQLWIZX2QFPNJAP7VUERCCRNGRHWZG3YYHIUV7BVDQ5FDPRT2@morlnodes.example.org",
	...
	var (
		...
		wantLinks = []string{"enrtree://AM5FCQLWIZX2QFPNJAP7VUERCCRNGRHWZG3YYHIUV7BVDQ5FDPRT2@morlnodes.example.org"}
		wantSeq   = uint(1)
	)
...
func TestClientSyncTreeBadNode(t *testing.T) {
	...
	// b.WriteString("-----")
	// badHash := subdomain(&b)
	// tree, _ := MakeTree(3, nil, []string{"enrtree://AM5FCQLWIZX2QFPNJAP7VUERCCRNGRHWZG3YYHIUV7BVDQ5FDPRT2@morlnodes.example.org"})
...
func updateSomeNodes(keySeed int64, nodes []*lnode.Node) {
	keys := testKeys(nodesSeed1, len(nodes))
	for i, n := range nodes[:len(nodes)/2] {
		r := n.Record()
		r.Set(enr.IP{127, 0, 0, 1})
		r.SetSeq(55)
		lnode.SignV4(r, keys[i])
		n2, _ := lnode.New(lnode.ValidSchemes, r)
		nodes[i] = n2
	}
}
...
func checkIterator(t *testing.T, it lnode.Iterator, wantNodes []*lnode.Node) {
	t.Helper()

	var (
		want     = make(map[lnode.ID]*lnode.Node)
...
func makeTestTree(domain string, nodes []*lnode.Node, links []string) (*Tree, string) {
...
func testNodes(seed int64, n int) []*lnode.Node {
	keys := testKeys(seed, n)
	nodes := make([]*lnode.Node, n)
	for i, key := range keys {
		record := new(enr.Record)
		record.SetSeq(uint64(i))
		lnode.SignV4(record, key)
		n, err := lnode.New(lnode.ValidSchemes, record)
		if err != nil {
			panic(err)
		}
		nodes[i] = n
...
func testNode(seed int64) *lnode.Node {
