...
func MakeTree(seq uint, nodes []*lnode.Node, links []string) (*Tree, error) {
	// Sort records by ID and ensure all nodes have a valid record.
	records := make([]*lnode.Node, len(nodes))
...
func sortByID(nodes []*lnode.Node) []*lnode.Node {
...
func parseENR(e string, validSchemes enr.IdentityScheme) (entry, error) {
	...
	n, err := lnode.New(validSchemes, &rec)
	if err != nil {
		return nil, entryError{"enr", err}
	}
