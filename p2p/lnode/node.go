...
// MustParse parses a node record or lnode:// URL. It panics if the input is invalid.
...
func Parse(validSchemes enr.IdentityScheme, input string) (*Node, error) {
	if strings.HasPrefix(input, "lnode://") {
...
// ValidateComplete checks whforter n has a valid IP and UDP port.
...
func (n ID) GoString() string {
	return fmt.Sprintf("lnode.HexID(\"%x\")", n[:])
}
