...
func NewPeer(id lnode.ID, name string, caps []Cap) *Peer {
	pipe, _ := net.Pipe()
...
func (p *Peer) ID() lnode.ID {
	return p.rw.node.ID()
}

// Node returns the peer's node descriptor.
func (p *Peer) Node() *lnode.Node {
	return p.rw.node
}
...
type PeerInfo struct {
	ENR     string   `json:"enr,omitempty"` // Luck Node Record
	Enode   string   `json:"lnode"`         // Node URL
