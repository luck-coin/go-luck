...
func TestServerListen(t *testing.T) {
	...
	srv := startTestServer(t, remid, func(p *Peer) {
		if p.ID() != lnode.PubkeyToIDV4(remid) {
...
func TestServerDial(t *testing.T) {
	...
	node := lnode.NewV4(remid, tcpAddr.IP, tcpAddr.Port, 0)
	srv.AddPeer(node)
	...
	case conn := <-accepted:
		...
		select {
		case peer := <-connected:
			if peer.ID() != lnode.PubkeyToIDV4(remid) {
...
func TestServerAtCap(t *testing.T) {
	trustedNode := newkey()
	trustedID := lnode.PubkeyToIDV4(&trustedNode.PublicKey)
	srv := &Server{
		Config: Config{
			PrivateKey:   newkey(),
			MaxPeers:     10,
			NoDial:       true,
			NoDiscovery:  true,
			TrustedNodes: []*lnode.Node{newNode(trustedID, "")},
			Logger:       testlog.Logger(t, log.LvlTrace),
		},
	}
	...
	newconn := func(id lnode.ID) *conn {
		fd, _ := net.Pipe()
		tx := newTestTransport(&trustedNode.PublicKey, fd)
		node := lnode.SignNull(new(enr.Record), id)
...
func TestServerPeerLimits(t *testing.T) {
	srvkey := newkey()
	clientkey := newkey()
	clientnode := lnode.NewV4(&clientkey.PublicKey, nil, 0, 0)
...
func TestServerSetupConn(t *testing.T) {
	...
	tests := []struct {
		...
		{
			tt:           &setupTransport{pubkey: clientpub},
			dialDest:     lnode.NewV4(&newkey().PublicKey, nil, 0, 0),
			flags:        dynDialedConn,
			wantCalls:    "doEncHandshake,close,",
		...
		},
		{
			...
			dialDest:     lnode.NewV4(clientpub, nil, 0, 0),
			flags:        dynDialedConn,
...
func syncAddPeer(srv *Server, node *lnode.Node) bool {
	var (
