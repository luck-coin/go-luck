...
import (
	...
	"github.com/luck/go-luck/p2p/simulations/pipes"
...
func NewSimAdapter(services map[string]ServiceFunc) *SimAdapter {
	return &SimAdapter{
		pipe:     pipes.NetPipe,
		nodes:    make(map[lnode.ID]*SimNode),
		services: services,
	}
}
...
func (s *SimAdapter) Dial(ctx context.Context, dest *lnode.Node) (conn net.Conn, err error) {
...
func (s *SimAdapter) DialRPC(id lnode.ID) (*rpc.Client, error) {
...
func (s *SimAdapter) GetNode(id lnode.ID) (*SimNode, bool) {
	s.mtx.RLock()
	defer s.mtx.RUnlock()
	node, ok := s.nodes[id]
	return node, ok
...
func (sn *SimNode) Node() *lnode.Node {
