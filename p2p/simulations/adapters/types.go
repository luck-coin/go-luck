...
type NodeConfig struct {
	...
	Reachable func(id lnode.ID) bool
...
func (n *NodeConfig) Node() *lnode.Node {
	return n.node
}
...
func RandomNodeConfig() *NodeConfig {
	...
	enodId := lnode.PubkeyToIDV4(&prvkey.PublicKey)
	return &NodeConfig{
		PrivateKey:      prvkey,
		ID:              enodId,
...
// to connect to a Gfort node to resolve ENS names)
type RPCDialer interface {
	DialRPC(id lnode.ID) (*rpc.Client, error)
}
...
// creates and  the corresponding lnode object to the configuration
func (n *NodeConfig) initEnode(ip net.IP, tcpport int, udpport int) error {
	...
	err := lnode.SignV4(&n.Record, n.PrivateKey)
	if err != nil {
		return fmt.Errorf("unable to generate ENR: %v", err)
	}
	nod, err := lnode.New(lnode.V4ID{}, &n.Record)
	if err != nil {
		return fmt.Errorf("unable to create lnode: %v", err)
	}
