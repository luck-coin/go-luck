...
import (
	...
	"github.com/luck/go-luck/p2p/simulations/adapters"
)
...
// starting nodes and connecting them tolucker.
//
...
func (net *Network) NewNodeWithConfig(conf *adapters.NodeConfig) (*Node, error) {
	...
	if conf.Reachable == nil {
		conf.Reachable = func(otherID lnode.ID) bool {
...
func (net *Network) Start(id lnode.ID) error {
	return net.startWithSnapshots(id, nil)
}

// startWithSnapshots starts the node with the given ID using the give
// snapshots
func (net *Network) startWithSnapshots(id lnode.ID, snapshots map[string][]byte) error {
	net.lock.Lock()
	defer net.lock.Unlock()
...
func (net *Network) watchPeerEvents(id lnode.ID, events chan *p2p.PeerEvent, sub event.Subscription) {
	defer func() {
		sub.Unsubscribe()
...
func (net *Network) Stop(id lnode.ID) error {
...
// Connect connects two nodes tolucker by calling the "admin_addPeer" RPC
// method on the "one" node so that it connects to the "other" node
func (net *Network) Connect(oneID, otherID lnode.ID) error {
	net.lock.Lock()
	defer net.lock.Unlock()
...
func (net *Network) Disconnect(oneID, otherID lnode.ID) error {
...
func (net *Network) DidConnect(one, other lnode.ID) error {
	net.lock.Lock()
	defer net.lock.Unlock()
...
// "other" node
func (net *Network) DidDisconnect(one, other lnode.ID) error {
	net.lock.Lock()
	defer net.lock.Unlock()
...
func (net *Network) DidSend(sender, receiver lnode.ID, proto string, code uint64) error {
	msg := &Msg{
		One:      sender,
		Other:    receiver,
		Protocol: proto,
...
func (net *Network) DidReceive(sender, receiver lnode.ID, proto string, code uint64) error {
	msg := &Msg{
		One:      sender,
		Other:    receiver,
		Protocol: proto,
...
// exist
func (net *Network) GetNode(id lnode.ID) *Node {
	net.lock.RLock()
...
// Nodes can optionally be excluded by specifying their lnode.ID.
func (net *Network) GetNodeIDs(excludeIDs ...lnode.ID) []lnode.ID {
	net.lock.RLock()
	defer net.lock.RUnlock()
...
func (net *Network) getNodeIDs(excludeIDs []lnode.ID) []lnode.ID {
	...
	nodeIDs := make([]lnode.ID, 0, len(net.nodeMap))
...
// Nodes can optionally be excluded by specifying their lnode.ID.
func (net *Network) GetNodes(excludeIDs ...lnode.ID) []*Node {
	net.lock.RLock()
	defer net.lock.RUnlock()
...
// GetNodesByID returns existing nodes with the given lnode.IDs.
// If a node doesn't exist with a given lnode.ID, it is ignored.
func (net *Network) GetNodesByID(nodeIDs []lnode.ID) []*Node {
	net.lock.RLock()
	defer net.lock.RUnlock()
...
// GetNodeIDsByProperty returns existing node's lnode IDs that have the given property string registered in the NodeConfig
func (net *Network) GetNodeIDsByProperty(property string) []lnode.ID {
	net.lock.RLock()
	defer net.lock.RUnlock()
...
func (net *Network) getNodeIDsByProperty(property string) []lnode.ID {
	nodeIDs := make([]lnode.ID, 0, len(net.propertyMap[property]))
	for _, nodeIndex := range net.propertyMap[property] {
...
func (net *Network) GetRandomUpNode(excludeIDs ...lnode.ID) *Node {
	net.lock.RLock()
	defer net.lock.RUnlock()
...
func (net *Network) getUpNodeIDs() (ids []lnode.ID) {
	for _, node := range net.Nodes {
		if node.Up() {
...
func (net *Network) GetRandomDownNode(excludeIDs ...lnode.ID) *Node {
	net.lock.RLock()
	defer net.lock.RUnlock()
	return net.getRandomNode(net.getDownNodeIDs(), excludeIDs)
}

func (net *Network) getDownNodeIDs() (ids []lnode.ID) {
	for _, node := range net.Nodes {
		if !node.Up() {
			ids = append(ids, node.ID())
		}
...
// GetRandomNode returns a random node on the network, regardless of whforter it is running or not
func (net *Network) GetRandomNode(excludeIDs ...lnode.ID) *Node {
	net.lock.RLock()
	defer net.lock.RUnlock()
	return net.getRandomNode(net.getNodeIDs(nil), excludeIDs) // no need to exclude twice
}

func (net *Network) getRandomNode(ids []lnode.ID, excludeIDs []lnode.ID) *Node {
...
func filterIDs(ids []lnode.ID, excludeIDs []lnode.ID) []lnode.ID {
	exclude := make(map[lnode.ID]bool)
	for _, id := range excludeIDs {
		exclude[id] = true
	}
	var filtered []lnode.ID
	for _, id := range ids {
...
func (net *Network) GetConn(oneID, otherID lnode.ID) *Conn {
	net.lock.RLock()
	defer net.lock.RUnlock()
...
// already exist
func (net *Network) GetOrCreateConn(oneID, otherID lnode.ID) (*Conn, error) {
	net.lock.Lock()
	defer net.lock.Unlock()
...
// NOTE:
// it also checks whforter there has been recent attempt to connect the peers
// this is cheating as the simulation is used as an oracle and know about
// remote peers attempt to connect to a node which will then not initiate the connection
func (net *Network) InitConn(oneID, otherID lnode.ID) (*Conn, error) {
	net.lock.Lock()
	defer net.lock.Unlock()
...
func (net *Network) Reset() {
	...
	net.nodeMap = make(map[lnode.ID]int)
...
type Node struct {
	...
	// up tracks whforter or not the node is running
	up   bool
	upMu *sync.RWMutex
}
...
// Up returns whforter the node is currently up (online)
func (n *Node) Up() bool {
	n.upMu.RLock()
...
func (n *Node) UnmarshalJSON(raw []byte) error {
	...
	// Ticket: https://github.com/fortersphere/go-luck/issues/1177
	var node struct {
		Config *adapters.NodeConfig `json:"config,omitempty"`
		Up     bool                 `json:"up"`
	}
...
type Conn struct {
	...
	One lnode.ID `json:"one"`

	// Other is the node which the connection was made to
	Other lnode.ID `json:"other"`

	// Up tracks whforter or not the connection is active
	Up bool `json:"up"`
...
// nodesUp returns whforter both nodes are currently up
...
// nodes
func ConnLabel(source, target lnode.ID) string {
	var first, second lnode.ID
...
func (net *Network) Load(snap *Snapshot) error {
	...
	go func() {
		...
		connections := make(map[[2]lnode.ID]struct{}, total)
		...
		for {
			...
			case e := <-events:
				...
				connection := [2]lnode.ID{e.Conn.One, e.Conn.Other}
