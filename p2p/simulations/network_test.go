...
func TestSnapshot(t *testing.T) {
	...
	defer cancel()
	checkIds := make(map[lnode.ID][]lnode.ID)
...
func TestNetworkSimulation(t *testing.T) {
	...
	check := func(ctx context.Context, id lnode.ID) (bool, error) {
...
func triggerChecks(ctx context.Context, ids []lnode.ID, trigger chan lnode.ID, interval time.Duration) {
...
func benchmarkMinimalServiceTmp(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		protoCMap := make(map[lnode.ID]map[lnode.ID]chan struct{})
		adapter := adapters.NewSimAdapter(adapters.Services{
			"noopwoop": func(ctx *adapters.ServiceContext) (node.Service, error) {
				protoCMap[ctx.Config.ID] = make(map[lnode.ID]chan struct{})
...
func casesNodeUnmarshalJSONUpField() []nodeUnmarshalTestCase {
	return []nodeUnmarshalTestCase{
		...
		{
			name:      "bool value expected but got somforting else (string)",
			marshaled: "{\"up\": \"true\"}",
