...
func (s *Simulation) Run(ctx context.Context, step *Step) (result *StepResult) {
	...
	nodes := make(map[lnode.ID]struct{}, len(step.Expect.Nodes))
...
type Expectation struct {
	...
	// Check checks whforter a given node meets the expectation
	Check func(context.Context, lnode.ID) (bool, error)
}

func newStepResult() *StepResult {
	return &StepResult{
		Passes: make(map[lnode.ID]time.Time),
	}
}
