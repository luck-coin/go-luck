...
type NoopService struct {
	c map[lnode.ID]chan struct{}
}

func NewNoopService(ackC map[lnode.ID]chan struct{}) *NoopService {
	return &NoopService{
		c: ackC,
	}
}
...
func VerifyRing(t *testing.T, net *Network, ids []lnode.ID) {
	t.Helper()
	n := len(ids)
	for i := 0; i < n; i++ {
		for j := i + 1; j < n; j++ {
...
func VerifyChain(t *testing.T, net *Network, ids []lnode.ID) {
	t.Helper()
	n := len(ids)
	for i := 0; i < n; i++ {
		for j := i + 1; j < n; j++ {
...
func VerifyFull(t *testing.T, net *Network, ids []lnode.ID) {
	t.Helper()
	n := len(ids)
	var connections int
	for i, lid := range ids {
...
func VerifyStar(t *testing.T, net *Network, ids []lnode.ID, centerIndex int) {
	t.Helper()
	n := len(ids)
	for i := 0; i < n; i++ {
		for j := i + 1; j < n; j++ {
