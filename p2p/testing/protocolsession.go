...
type Trigger struct {
	...
	Peer    lnode.ID      // the peer to send the message to
...
func (s *ProtocolSession) expect(exps []Expect) error {
	...
	peerExpects := make(map[lnode.ID][]Expect)
	...
	mockNodes := make(map[lnode.ID]*mockNode)
...
func (s *ProtocolSession) TestDisconnected(disconnects ...*Disconnect) error {
	expects := make(map[lnode.ID]error)
