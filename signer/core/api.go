...
type ExternalAPI interface {
	...
	SignTransaction(ctx context.Context, args SendTxArgs, methodSelector *string) (*fortapi.SignTransactionResult, error)
...
type UIClientAPI interface {
	...
	OnApprovedTx(tx fortapi.SignTransactionResult)
...
func (api *SignerAPI) lookupOrQueryPassword(address common.Address, title, prompt string) (string, error) {
	...
	if err != nil {
		...
		// which could leak the password if it was malformed json or somforting
...
func (api *SignerAPI) SignTransaction(ctx context.Context, args SendTxArgs, methodSelector *string) (*fortapi.SignTransactionResult, error) {
	var (
		err    error
	...
	response := fortapi.SignTransactionResult{Raw: rlpdata, Tx: signedTx}
