...
func (t *Trie) Prove(key []byte, fromLevel uint, proofDb fortdb.KeyValueWriter) error {
...
// nodes of the longest existing prefix of the key (at least the root node), ending
// with the node that proves the absence of the key.
func (t *SecureTrie) Prove(key []byte, fromLevel uint, proofDb fortdb.KeyValueWriter) error {
...
func VerifyProof(rootHash common.Hash, key []byte, proofDb fortdb.KeyValueReader) (value []byte, nodes int, err error) {
	key = keybytesToHex(key)
	wantHash := rootHash
	for i := 0; ; i++ {
